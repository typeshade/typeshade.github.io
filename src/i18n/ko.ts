...
export const ko: Copy = {
  ...
  concepts: {
    ...
    cpuAndGpu: {
      ...
      description: 'GPU가 인보케이션 하나에 무엇을 건네는지, 셰이더에 힙과 문자열과 재귀가 없는 이유는 무엇인지, 그 사실에서 어떤 TypeShade 규칙이 나오는지 설명합니다.',
      h1: 'CPU와 GPU',
      ...
      invocationH: '인보케이션',
      invocationP: 'TypeScript 함수는 호출할 때 한 번 실행됩니다. 진입점은 다릅니다. GPU가 부르며, 한 번 그릴 때 버텍스마다 한 번, 프리미티브가 덮는 프래그먼트마다 한 번, 디스패치의 작업 항목마다 한 번 실행됩니다. 이렇게 한 번 실행되는 단위를 인보케이션이라고 부릅니다. 인보케이션은 서로 나란히 돌아가며 서로의 값을 읽지 못합니다. 자기가 몇 번째인지 알려 주는 것도 함수 바깥에는 없습니다.',
      ...
      loopsH: '루프',
      loopsP: '한 스테이지의 인보케이션들은 루프를 함께 지나가고, 먼저 빠져나온 쪽은 나머지를 기다립니다. 컴파일러가 읽을 수 있는 상한이 있어야 그 기다림의 크기를 가늠할 수 있고, 타깃이 요구할 때 백엔드가 본문을 펼칠 수도 있습니다.',
      loopsRule: '여기에서 규칙이 나옵니다. 루프는 컴파일러가 이미 아는 값을 셉니다. 어떤 조건과 반복이 컴파일되는지는 [제어 흐름](languageControlFlow)이 적어 두었습니다.',
      typesH: '값 타입',
      typesP: 'GPU 레지스터의 폭과 배치는 셰이더를 컴파일할 때 정해집니다. 그래서 변수는 선언된 자리부터 스코프 끝까지 값 타입 하나를 담습니다. 값 타입 두 개를 합친 유니온도 없고, 실행 중에 둘 사이를 고를 방법도 없습니다.',
      typesRule: '여기에서 규칙이 나옵니다. 모든 값은 `f32`, `u32`, `vec4`처럼 적어 둔 GPU 타입을 가집니다. 타입 작성 인터페이스는 [타입](languageTypes)이, 값 자체는 [GPU 타입](languageGpuTypes)이 적어 두었습니다.',
      tableH: '사실과 규칙',
      tableP: '위의 사실들과 거기에서 나오는 규칙, 그리고 그 규칙을 적어 둔 언어 가이드의 페이지입니다.',
      ...
      nextP: '이 경로의 다음 글은 [파이프라인](conceptsPipeline)입니다. 스테이지마다 무엇을 건네받고 무엇을 만들어 내는지 다룹니다.'
    },

    pipeline: {
      title: '셰이더 파이프라인 구조',
      description: '버텍스 스테이지, 프래그먼트 스테이지, 컴퓨트 스테이지가 각각 무엇을 건네받고 무엇을 만들어 내는지, 그리고 진입점 시그니처가 그 자리에 어떻게 대응하는지 설명합니다.',
      h1: '파이프라인',
      ...
      entryH: '진입점',
      entryP: '진입점의 시그니처는 그 스테이지 인터페이스를 그대로 적어 둔 것입니다. `@builtin(...)` 매개변수는 스테이지가 인보케이션에게 건네는 값이며, 버텍스 번호나 프래그먼트 위치가 여기에 해당합니다. 구조체 매개변수는 버텍스 스테이지에서는 버텍스마다의 입력이고, 프래그먼트 스테이지에서는 보간된 값입니다. 반환 타입은 스테이지가 파이프라인에 돌려주는 값이라서, 버텍스 진입점은 위치를 돌려주고 프래그먼트 진입점은 색을 돌려줍니다. 데코레이터와 표기는 [셰이더 스테이지](languageStages)에 있고, 이 페이지는 의미만 다룹니다.',
      ...
      nextP: '이 경로의 다음 글은 [WebGPU와 WebGL2](conceptsWebgpu)입니다. 호스트 애플리케이션과 컴파일러가 일을 어떻게 나누는지 다룹니다.'
    },

    webgpuAndWebgl2: {
      title: 'WebGPU와 WebGL2',
      description: '호스트 애플리케이션이 소유하는 것과 TypeShade가 소유하는 것, 컴파일러의 리플렉션이 바인드 그룹 레이아웃으로 이어지는 경로, 그리고 WebGL2에서 달라지는 지점을 설명합니다.',
      h1: 'WebGPU와 WebGL2',
      ...
      ownsRows: [
        ['디바이스', '어댑터와 `GPUDevice`를 요청하고 페이지가 살아 있는 동안 들고 있습니다.', '없습니다. TypeShade 코드는 WebGPU 객체를 건드리지 않습니다.'],
        ['파이프라인', '렌더 파이프라인이나 컴퓨트 파이프라인을 만들고 스테이지마다 진입점 이름을 지정합니다.', '모듈의 셰이더 텍스트와, 그 안의 진입점 이름을 모두 제공합니다.'],
        ['바인드 그룹 레이아웃', '바인딩마다 그룹, 번호, 종류, 그리고 그 바인딩을 보는 스테이지를 적습니다.', '`reflect()`가 컴파일된 모듈에서 같은 내용을 읽어 돌려줍니다.'],
        ['버퍼', '버퍼를 할당하고 바이트를 씁니다.', '유니폼 구조체의 필드마다 오프셋과 크기와 타입을 알려 줍니다.'],
        ['텍스처와 샘플러', '둘을 만들어 바인드 그룹에 넣습니다.', '셰이더가 선언한 바인딩과, 거기에서 기대하는 타입을 알려 줍니다.']
      ],
      reflectionH: '리플렉션',
      ...
      webgl2Items: [
        ...
        'GPU 기능은 두 번 켜야 합니다. 호스트가 컨텍스트에 확장을 요청하고, 생성된 소스도 그 확장을 선언합니다. 컴파일러가 이 두 몫을 어떻게 나누는지는 [컴파일러 내부](internals)가 설명합니다.'
      ],
      furtherH: '더 읽을 자료',
      furtherItems: [
      ...
      nextP: '이 경로의 다음 글은 [WGSL과 GLSL](conceptsWgsl)입니다. 소스 하나가 두 타깃에서 무엇으로 컴파일되는지 그대로 보여 줍니다.'
    },

    wgslAndGlsl: {
      title: `WGSL과 ${glsl}`,
      description: 'TypeShade 소스 하나가 컴파일되는 두 셰이더 언어, 작은 셰이더 하나의 생성 결과, 그리고 두 타깃이 갈라지는 지점을 설명합니다.',
      h1: `WGSL과 ${glsl}`,
      ...
      sourceH: '소스',
      sourceP: '삼각형 하나입니다. 버텍스 진입점이 위치를 정하고 프래그먼트 진입점이 색을 칠합니다. 구조체 두 개가 스테이지마다 출력 모양을 지정합니다.',
      sourceLabel: 'hello.shade.ts',
      wgslH: 'WGSL',
      wgslP: 'WGSL은 WebGPU 디바이스가 받아들이는 언어이고, 모듈 하나가 모든 스테이지를 담습니다. 구조체는 구조체로 남고, 진입점은 각자의 스테이지 속성을 그대로 지니며, builtin 입력은 그 값을 받는 매개변수의 속성으로 남습니다.',
      wgslLabel: '생성된 WGSL 모듈',
      glslH: `${glsl}`,
      glslP: 'WebGL2 프로그램은 버텍스 셰이더 하나와 프래그먼트 셰이더 하나를 링크해 만들기 때문에, 컴파일러는 스테이지마다 별도의 프로그램을 생성합니다. 각 프로그램은 버전 줄과 기본 정밀도로 시작하고, 버텍스 번호는 언어가 예약해 둔 이름으로 들어오며, 프래그먼트 출력은 선언된 out 변수가 됩니다.',
      ...
      diffRows: [
        ['[셰이더 스테이지](languageStages)', '`@compute` 진입점은 워크그룹 크기를 담은 컴퓨트 셰이더를 생성합니다.', '이 타깃에는 컴퓨트 스테이지가 없어서, 컴퓨트를 선언한 모듈은 WGSL만 생성합니다.'],
        ['[정밀도](languageGpuTypes)', '타입이 자기 폭을 지니므로 프로그램은 아무것도 선언하지 않습니다.', '프로그램이 부동소수점과 정수의 기본 정밀도를 먼저 선언합니다.'],
        ['[builtin 입력](languageStages)', 'builtin은 그 값을 받는 매개변수의 속성으로 남습니다.', 'builtin은 언어가 예약해 둔 이름이 되고, 매개변수는 사라집니다.'],
        ['[스테이지 출력](languageTypes)', '스테이지는 필드마다 location을 지닌 구조체를 돌려줍니다.', '스테이지는 선언된 out 변수에 쓰고, 위치는 예약된 변수로 갑니다.'],
        ['[확장](internals)', 'GPU 기능은 모듈 맨 위의 선언으로 켭니다.', 'GPU 기능은 전처리기 줄로 켜고, 호스트도 컨텍스트에 해당 확장을 요청합니다.']
      ],
      furtherH: '더 읽을 자료',
      furtherItems: [
      ...
      nextP: '다음은 [예제](examples)입니다. 컴파일러 레지스트리의 예제마다 어떤 타깃을 생성하는지 적혀 있습니다.'
    },
  },

  languageService: {
    title: '에디터와 언어 서버가 쓰는 TypeShade 언어 서비스',
    description: 'Playground 뒤에서 일하는 에디터 중립 계층입니다. TypeScript와 TypeShade 진단, 자동 완성, 호버, 이름 바꾸기, 컴파일 결과를 문서 API 하나로 답합니다.',
    h1: '언어 서비스',
    intro: '컴파일러 프런트엔드와 에디터 사이에는 텍스트와 위치를 받아 데이터를 돌려주는 계층이 있습니다. 이것이 언어 서비스입니다. DOM이나 Node API는 건드리지 않습니다. 지금은 Playground가 Monaco 에디터를 통해 진단, 자동 완성, 호버를 여기에서 읽고, 나중에는 VS Code와 다른 에디터를 위한 언어 서버가 같은 계층을 읽습니다. 그래서 둘이 서로 어긋날 수 없습니다.',
    layersH: '계층',
    layersP: '프런트엔드는 `"use typeshade"` 파일을 파싱하고 타입, 구조체, 바인딩을 검사한 뒤 소스 위치가 붙은 진단을 보고합니다. 언어 서비스는 두 층 위에 놓입니다. 하나는 그 프런트엔드이고, 다른 하나는 TypeShade 전역을 선언한 앰비언트 선언 위에서 돌아가는 TypeScript 언어 서비스입니다. 문서를 `uri`로 들고 있다가 그 문서에 관한 요청에 답합니다. 어댑터는 그 위에서 의미와 무관한 일만 맡습니다. Playground의 Monaco 어댑터는 좌표를 바꾸고 에디터의 마커를 관리하며, LSP 서버는 같은 답을 JSON-RPC로 옮길 뿐입니다. TypeShade에 관한 판단은 서비스 안에 두고, 어댑터는 형식만 바꿉니다.',
    requestsH: '요청',
    requestsP: '에디터가 보내는 요청은 문서 API 하나로 답합니다.',
    requests: [
      ['진단', 'TypeScript 진단과 TypeShade 진단을 한 목록으로 돌려줍니다. 항목마다 `typeshade` 또는 `typescript`라는 `source`와 코드가 붙어 있어 어댑터가 둘을 구분할 수 있고, TypeScript 구문 오류는 TypeScript 코드로 한 번만 나타납니다.'],
      ...
      ['호버', '심볼의 요약 정보를 보여 줍니다. TypeScript가 `number`라고 할 자리에 TypeShade 타입 이름을 쓰고, GPU 타입, 속성, 내장 입력의 설명을 붙입니다.'],
    ...
    documentsH: '문서와 위치',
    documentsP1: '`typeshade/language-service` 하위 경로에서 `createTypeshadeLanguageService`를 가져오고, 문서를 `uri`와 텍스트, 선택적인 버전으로 엽니다. 내용이 바뀔 때마다 전체 텍스트로 갱신하고, 에디터가 문서를 닫으면 함께 닫습니다. 다른 메서드는 모두 `uri`를 받고, 필요한 경우 위치를 함께 받습니다. 서비스 안에는 비동기 동작이 없으며, 오래된 버전의 결과를 버리는 일은 어댑터가 맡습니다.',
    documentsP2: '위치는 줄과 문자의 쌍이며 둘 다 영에서 시작하고, 문자는 UTF-16 코드 단위로 셉니다. 범위는 끝을 포함하지 않는 반개구간입니다. LSP가 쓰는 규약 그대로이므로 언어 서버는 필드를 그대로 넘기면 됩니다. Monaco는 하나부터 세기 때문에 Playground의 어댑터가 자기 쪽에서 하나를 더하고, 돌아올 때 다시 뺍니다. 두 좌표계가 만나는 곳은 그 어댑터 하나뿐입니다.',
    packagingH: '패키지 구성',
    packagingP: '패키지 가운데 `typescript`가 필요한 부분은 이 서비스 하나이고, 그 TypeScript 패키지는 선택적인 피어 의존성입니다. 컴파일러만 가져오는 프로그램은 추가로 설치할 것이 없고, `typeshade/language-service`를 가져오는 프로그램은 TypeScript를 직접 설치합니다.',
    exampleH: '예제',
    exampleP: '호스트가 문서 하나를 열고, 진단과 한 위치의 호버를 요청하고, 출력 창을 위해 컴파일합니다.',
    furtherH: '더 읽을 자료',
    furtherP: '[Playground](playground)는 브라우저에서 돌아가는 이 서비스입니다. 컴파일러 저장소의 [설계 문서](languageServiceDesign)는 고정한 커밋 기준으로 규약, 어댑터 계약, 작업 순서를 기록합니다.',
  },
  ...
  language: {
    ...
    topics: {
      ...
      functions: {
        ...
        paramsTable: [['부분', '역할'], ['`a`, `b`', 'GPU 입력 값입니다.'], ['`amount: f32`', '스칼라 입력이며 컴파일러가 연산 타입을 확인합니다.'], ['`: vec4`', '호출자에게 반환할 GPU 값의 shape입니다.']],
        ...
        builtinTable: [['표현', '의미'], ['`@builtin("global_invocation_id")`', 'GPU가 제공하는 compute 입력을 지정합니다.'], ['`gid: vec3u`', '그 입력의 TypeShade 타입과 local 이름입니다.'], ['`gid.x`', '현재 invocation의 x component를 읽습니다.']],
      ...
      gpuTypes: {
        ...
        live: '6. 라이브 예제',
        liveP: '아래 진입점은 줄마다 GPU 타입이 하나씩 나옵니다. 위치는 `vec2`, 거리와 반지름은 `f32`, 색은 `vec3`입니다.',
        liveTitle: '원반',
