    ...
    reference: {
      ...
      kinds: {
        ...
        attribute: {
          ...
          description: `컴파일러가 실제로 읽고 처리하는 데코레이터 ${facts.languageAttributes}개입니다. 셋은 진입점을 표시하고, 나머지 둘은 필드나 매개변수를 파이프라인의 자리에 연결합니다.`,
        },
        builtin: {
          name: '내장 값',
  ...
  language: {
    ...
    mapping: {
      ...
      fromWgsl: {
        ...
        refusals: {
          'scalars.f16': '이 인터페이스에는 이름이 없습니다. WGSL은 모듈에 `enable f16;`를, 디바이스에는 `shader-f16` 기능을 요구하는데 `"use typeshade"` 파일에는 둘 중 어느 쪽도 요청할 방법이 없습니다.',
        ...
        notes: {
          ...
          'statements.switch': 'case는 TypeScript가 요구하는 `break`로 끝내고, 하향 변환이 그것을 떼어 냅니다. 어느 case도 다음으로 흘러가지 않으며, 레이블은 한 번만 쓸 수 있는 정수 상수입니다.',
      ...
      fromGlsl: {
        ...
        enablesP: '`"use typeshade"` 파일에는 모듈이 이 기능들을 선언하는 목록을 적을 방법이 없습니다. 모듈의 모양에서 저절로 따라오는 셋, 곧 스토리지 바인딩과 컴퓨트 진입점과 멀티샘플 텍스처 읽기는 파일에서 도출되고, 나머지는 `fn()` 인터페이스를 거쳐 컴파일러에 닿습니다.',
