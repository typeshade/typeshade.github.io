...
export const ko: Copy = {
  ...
  languageService: {
    title: 'TypeShade 언어 서비스: 에디터와 언어 서버가 쓰는 계층',
    description: 'Playground 뒤에서 일하는 에디터 중립 계층입니다. TypeScript와 TypeShade 진단, 완성, 호버, 이름 바꾸기, 컴파일 결과를 문서 API 하나로 답합니다.',
    h1: '언어 서비스',
    intro: '컴파일러 프런트엔드와 에디터 사이에는 텍스트와 위치를 받아 데이터를 돌려주는 계층이 있습니다. 이것이 언어 서비스입니다. DOM이나 Node API는 건드리지 않습니다. 지금은 Playground가 Monaco 에디터를 통해 진단, 완성, 호버를 여기에서 읽고, 나중에는 VS Code와 다른 에디터를 위한 언어 서버가 같은 계층을 읽습니다. 그래서 둘이 서로 어긋날 수 없습니다.',
    layersH: '계층',
    layersP: '프런트엔드는 `"use typeshade"` 파일을 파싱하고 타입, 구조체, 바인딩을 검사한 뒤 소스 위치가 붙은 진단을 보고합니다. 언어 서비스는 그 프런트엔드 위에, 그리고 TypeShade 전역을 선언한 앰비언트 선언 위에서 돌아가는 TypeScript 언어 서비스 위에 놓입니다. 문서를 `uri`로 들고 있다가 그 문서에 관한 요청에 답합니다. 어댑터는 그 위에서 의미와 무관한 일만 맡습니다. Playground의 Monaco 어댑터는 좌표를 바꾸고 에디터의 마커를 관리하며, LSP 서버는 같은 답을 JSON-RPC로 옮길 뿐입니다. TypeShade에 관한 판단은 서비스 안에 두고, 어댑터는 형식만 바꿉니다.',
    requestsH: '요청',
    ...
    requests: [
      ...
      ['완성', '범위 안의 심벌과 키워드, 그리고 문맥에 맞는 TypeShade 항목을 제안합니다. `@` 뒤에는 속성 이름, `@builtin("` 안에는 내장 입력 이름, 타입 자리에는 GPU 타입 이름, 그리고 벡터와 진입 함수의 스니펫입니다.'],
      ['호버', '심벌의 요약 정보를 보여 줍니다. TypeScript가 `number`라고 할 자리에 TypeShade 타입 이름을 쓰고, GPU 타입, 속성, 내장 입력의 설명을 붙입니다.'],
      ['시그니처 도움말', '커서 아래 함수의 시그니처와 지금 입력하는 매개변수입니다.'],
      ['정의와 참조', '심벌이 선언된 곳과 쓰인 곳을 서비스가 들고 있는 모든 문서에서 찾습니다.'],
      ['문서 심벌', '문서의 함수, 구조체, 필드, 리소스를 개요로 정리하고, 진입 함수에는 스테이지를 표시합니다.'],
      ['이름 바꾸기', '심벌을 쓰는 모든 문서에 적용할 편집 목록을 돌려줍니다. 그 위치의 이름을 바꿀 수 있는지 먼저 확인합니다.'],
      ['시맨틱 토큰', '문서 순서대로 늘어놓은 토큰입니다. GPU 타입, 진입 함수, 리소스, `@builtin(...)` 안의 이름을 그렇게 표시합니다.'],
    ...
    packagingH: '패키지 구성',
    packagingP: '패키지 가운데 `typescript`가 필요한 부분은 이 서비스 하나이고, 이것은 선택적인 피어 의존성입니다. 컴파일러만 가져오는 프로그램은 추가로 설치할 것이 없고, `typeshade/language-service`를 가져오는 프로그램은 TypeScript를 직접 설치합니다.',
    exampleH: '예제',
