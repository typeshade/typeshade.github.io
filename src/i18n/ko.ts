...
export const ko: Copy = {
  ...
  docs: {
    ...
    reference: {
      ...
      kinds: {
        ...
        attribute: {
          name: '어트리뷰트',
          summary: '진입점을 표시하고, 필드를 파이프라인에 연결하고, 그 필드의 성질을 정하는 데코레이터입니다.',
          description: `컴파일러가 실제로 읽고 처리하는 데코레이터 ${facts.languageAttributes}개입니다. 셋은 진입점을 표시하고, 둘은 필드를 파이프라인에 연결합니다. 셋은 그 필드의 성질을 정하고, 하나는 진단 규칙을 설정합니다.`,
        },
        builtin: {
          name: '내장 값',
  ...
  concepts: {
    ...
    cpuAndGpu: {
      ...
      memoryRule: '여기에서 규칙이 나옵니다. `new`는 정체성을 가진 객체가 아니라 값을 만들고, 클래스 필드는 컴파일러가 WGSL 규칙에 맞춰 정렬하며 그 오프셋은 `reflect()`가 알려 줍니다. 리소스는 모두 `declare`로 들어옵니다. 구조체 작성 인터페이스는 [타입](languageTypes)이, 선언은 [리소스](languageResources)가 적어 두었습니다.',
      callsH: '호출',
    ...
    pipeline: {
      ...
      interpolationNote: 'float 타입의 `@location` 필드는 따로 적지 않는 한 모두 이렇게 가중됩니다. `@interpolate("flat")`을 붙이면 모든 프래그먼트가 버텍스 하나의 값을 그대로 받습니다. 정수 필드는 적지 않아도 `flat`이 됩니다. 두 타깃 모두 정수를 가중할 수 없기 때문입니다. 필드 데코레이터는 [타입](languageTypes)이 적어 두었습니다.',
      computeH: '컴퓨트',
  ...
  language: {
    ...
    topics: {
      types: {
        ...
        attrsP: '필드는 `@location`이나 `@builtin`으로 파이프라인에 연결합니다. 필드의 성질을 정하는 것도 셋 더 있습니다. `@location` varying에는 `@interpolate`를, 위치에는 `@invariant`를, 프래그먼트 출력에는 `@blend_src`를 붙입니다. `@align`은 읽고 나서 거부하며 `@size`, `@offset`, `@ignore`는 아는 어트리뷰트가 아닙니다. 받아들이는 전체 집합은 `@vertex`, `@fragment`, `@compute`, `@builtin`, `@location`, `@interpolate`, `@invariant`, `@blend_src`, `@diagnostic`입니다.',
    ...
    mapping: {
      ...
      fromWgsl: {
        ...
        refusals: {
          'scalars.f16': '이 인터페이스에는 이름이 없습니다. 파일에서 `"use typeshade"` 아래에 `"enable f16"`을 적으면 `enable f16;`이 생성되고 디바이스에 `shader-f16` 기능을 요구하지만, 값을 선언할 타입은 여전히 없습니다.',
        ...
        notes: {
          ...
          'stages.compute': '목록이 워크그룹 크기입니다. 목록 없는 `@compute`는 `[1, 1, 1]`이고, `@compute({ workgroup: [64, 1, 1] })`도 같은 뜻입니다.',
          ...
          'statements.switch': 'case는 `break`로 끝내고, 하향 변환이 그것을 떼어 냅니다. 사이에 아무것도 없이 겹쳐 쓴 레이블은 절 하나가 되며, WGSL은 이를 `case 0, 1:`로 씁니다. 레이블은 한 번만 쓸 수 있는 정수 상수입니다.',
      ...
      fromGlsl: {
        ...
        enablesP: '`"use typeshade"` 파일에는 이 행들을 적을 방법이 없습니다. 파일의 `"enable ..."` 지시문은 WGSL 확장 이름을 받는데, 이 행들에는 그런 이름이 없기 때문입니다. 스토리지 바인딩, 컴퓨트 진입점, 멀티샘플 텍스처 읽기처럼 모듈의 모양에서 따라오는 기능은 파일에서 도출되고, 나머지는 `fn()` 인터페이스를 거쳐 컴파일러에 닿습니다.',
