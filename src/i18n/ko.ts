...
export const ko: Copy = {
  ...
  examples: {
    ...
    shade: {
      ...
      p: `같은 디렉터리에 있는 예제 ${facts.shadeExamples}개는 \`"use typeshade"\`로 시작하는 TypeScript 파일이고, \`compile()\`이 파일의 바이트를 그대로 읽어 컴파일합니다. 파일마다 언어의 한 부분을 보여 주므로 여기서도 그 기준으로 묶었습니다. 이 가운데 ${facts.shadeRenderable}개는 ${glsl} 형태가 있어 그림이 함께 붙고, 나머지는 WGSL만 생성합니다.`,
      groups: {
        stages: '스테이지와 IO 구조체',
        resources: '리소스',
        values: '값과 제어 흐름',
