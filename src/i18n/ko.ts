...
export const ko: Copy = {
  ...
  live: {
    ...
    hint: '코드를 고치면 캔버스가 따라갑니다.',
    compiling: '컴파일 중',
    loading: '컴파일러를 불러오는 중',
    clean: '진단 없음',
    keptFrame: '마지막으로 컴파일된 프레임을 캔버스가 그대로 두고 있습니다.',
    controls: '컨트롤',
    reserved: (fields: string) => `다음 값은 페이지가 매 프레임 채우므로 컨트롤이 없습니다: ${fields}.`,
    mouseUnits: '`mouse`는 캔버스 왼쪽 아래를 원점으로 0에서 1까지 가며, `uv`와 같은 공간입니다.',
    backend: {
      neutral: '페이지가 로드되면 캔버스가 백엔드를 고릅니다.',
      webgpu: 'WebGPU에서 실행 중.',
      webgl2: `출력된 ${glsl}로 WebGL2에서 실행 중.`,
      none: `이 브라우저에는 WebGPU도 WebGL2도 없습니다. 위 프레임은 빌드할 때 그린 것이고, 예제가 내는 WGSL과 ${glsl}은 출력 결과에 있습니다.`,
    },
    output: '출력 결과',
    wgsl: 'WGSL',
    glslVertex: `${glsl} 버텍스`,
    ...
    uniforms: '유니폼 블록',
    noUniforms: '이 예제는 유니폼 블록을 바인딩하지 않습니다.',
    field: '필드',
    type: '타입',
    offset: '오프셋',
    entries: '진입점',
  ...
  quickStart: {
    ...
    live: {
      h: '라이브 예제',
      p: '아래 파일은 이 페이지 안에서 컴파일합니다. 유니폼 블록을 스스로 선언하고, 0에서 1까지의 `uv`를 받아 픽셀 하나의 색을 돌려줍니다. 이를 호출하는 풀스크린 삼각형은 페이지가 붙입니다. 한 줄을 고치면 다음 입력에 캔버스가 따라오고, 컴파일러가 낸 결과는 출력 결과에 있습니다.',
      title: '줄무늬',
      caption: '위 파일이 만든, 캔버스를 가로지르는 사인파.',
      speed: '초당 파동 이동 거리',
      bands: '캔버스를 가로지르는 줄 개수',
      tint: '줄무늬 색',
    },
    status: {
      h: '상태',
  ...
  language: {
    ...
    topics: {
      ...
      gpuTypes: {
        ...
        live: '6. 라이브 예제',
        liveP: '아래 진입점은 줄마다 GPU 타입이 하나씩 나옵니다. `vec2`는 위치를, `f32`는 거리와 반지름을, `vec3`은 색을 담습니다. 패드가 `vec2`를 옮기고 슬라이더가 `f32`를 움직이며 색 선택기가 `vec3`의 세 채널을 씁니다. 컨트롤 아래에 있는 것이 곧 타입입니다.',
        liveTitle: '원반',
        liveCaption: 'vec2가 자리를 잡고 f32가 크기를 정하며 vec3이 색을 칠하는 원반.',
