...
export const ko: Copy = {
  ...
  docs: {
    ...
    api: {
      ...
      description: 'TypeShade 컴파일러의 공개 export를 하나씩 페이지로 정리했습니다. 구문, 매개변수, 반환값, 예제, 그리고 어느 대상에서 지원되는지를 담습니다.',
      h1: '컴파일러 API 참조',
      intro: `typeshade 패키지가 내보내는 export를 모두 모았습니다. 커밋 ${facts.pinnedCommit}의 컴파일러에서 뽑았습니다. 함수, 타입, 인터페이스, 클래스마다 페이지가 하나씩 있습니다.`,
      audience: '호스트 애플리케이션과 [`fn()`](apiFn) 빌더가 부르는 쪽입니다. 셰이더를 쓰는 사람은 [언어 참조](reference)를 보십시오.',
      reference: '참조',
      breadcrumbs: '현재 위치',
    ...
    reference: {
      ...
      audience: '셰이더를 쓰는 사람을 위한 참조입니다. 호스트 애플리케이션과 [`fn()`](apiFn) 빌더가 부르는 쪽은 [컴파일러 API 참조](api)에 있습니다.',
      reference: '참조',
      breadcrumbs: '현재 위치',
      note: `이름마다 붙은 시그니처와 설명은 커밋 ${facts.pinnedCommit}의 컴파일러 소스에서 그대로 가져온 영어 원문입니다.`,
      readingH: '항목 읽는 법',
      numberP: '매개변수나 반환 위치에 쓰인 `number`는 스칼라가 들어갈 자리입니다. 앰비언트 파일은 GPU 스칼라를 브랜드가 붙은 number로 선언하고 그 브랜드는 선택이라서, 리터럴도 브랜드가 붙은 스칼라도 이 자리에 들어갑니다.',
      widthP: '직접 쓰는 선언에는 여전히 너비가 필요합니다. 필드나 매개변수에 `number`만 적으면 거부되며, 그 행과 컴파일러가 낸 메시지는 [TypeScript에서](languageFromTypescript) 페이지에 있습니다.',
      positionsP: '`a0`, `a1`처럼 자리로만 붙은 매개변수 이름은 앰비언트 파일이 만들어 낸 것입니다. 호출은 인자를 순서대로 넘기고 이름은 쓰지 않습니다.',
      helpersP: '시그니처에는 앰비언트 파일이 스스로 쓰려고 선언한 타입이 나올 수 있습니다. 아래가 그 선언입니다.',
      ...
      sourceP: '이 페이지의 항목은 모두 고정된 커밋의 컴파일러에서 읽어 왔고, 여기서 직접 입력한 것은 없습니다.',
    ...
    errors: {
      ...
      verified: (n: number) => `${n}개 페이지에는 코드를 내는 프로그램과 그 프로그램을 고친 모습이 함께 실려 있습니다. 둘 다 빌드할 때 고정된 컴파일러로 컴파일하므로, 프로그램이 더는 그 코드를 내지 않거나 고친 프로그램에 진단이 남으면 빌드가 멈춥니다. 예제 태그가 붙은 행이 그런 페이지입니다.`,
      note: '레지스트리 설명과 컴파일러 메시지는 컴파일러 원문이라 한국어 페이지에서도 영어로 둡니다.',
      codes: (n: number) => `코드 ${n}개`,
      frontEndH: '프런트엔드 코드',
      frontEndP: '`TS80xx` 코드는 프런트엔드의 `TS_CODES`에 들어 있습니다. `"use typeshade"` 파일은 IR을 만들기 전에 이 코드들로 먼저 검사받습니다.',
      coreH: '코어 코드',
      coreP: '`SDxxxx` 코드는 `typeshade/dev`가 내보내는 `CODES` 카탈로그에 들어 있습니다. 코어 IR 계층은 [`fn()`](apiFn) 빌더에서도 `"use typeshade"` 파일에서도 이 코드를 냅니다. 다만 같은 실수 가운데 상당수는 프런트엔드가 자기 코드로 먼저 거부합니다.',
      numberingH: '번호 매기기',
      numberingP: '프런트엔드 레지스트리는 번호를 매기는 방식을 파일 머리 주석에 직접 적어 두었습니다.',
      exampleTag: '예제',
      groups: {
        file: '파일',
        types: '타입과 이름',
      ...
      entry: {
        ...
        description: (head: string, line: string) => `${head}, TypeShade 컴파일러의 오류 코드입니다. ${line}`,
        fill: '이 페이지에는 레지스트리 원문과 컴파일러가 이 코드를 내는 곳, 고치는 법이 있습니다.',
        kindTs: '프런트엔드 코드',
        kindSd: '코어 코드',
        kindRetired: '폐기된 번호',
        kindMeta: '종류',
        undocumented: '레지스트리에는 이 상수에 대한 문서 주석이 없습니다. 그래서 위 설명은 컴파일러가 이 코드와 함께 내는 메시지를 보고 이 사이트가 쓴 것입니다.',
        whenH: '발생 조건',
        exampleH: '예제',
        fixH: '고치는 법',
        frontEndH: '프런트엔드에서',
        ...
        backend: '빌드할 때 고정된 컴파일러로 이 프로그램을 컴파일하면 `TS8015` 진단이 나옵니다. 백엔드가 모듈을 거부했다고 프런트엔드가 알리는 진단입니다. 백엔드가 던진 오류에 이 코드가 들어 있습니다.',
        diagnose: '`compile()`은 린트 규칙을 돌리지 않습니다. `typeshade/dev`의 `diagnose()`가 `compile()`이 돌려준 모듈에 린트 규칙을 돌려 다음을 알립니다.',
        deprecations: '이 경고를 켜는 옵션인 `{ deprecations: true }`를 주고 컴파일했습니다.',
        hint: '레지스트리의 힌트:',
        fixed: '같은 프로그램을 고친 모습입니다. 진단 없이 컴파일됩니다.',
        fixedDiagnose: '같은 프로그램을 고친 모습입니다. 진단 없이 컴파일되고, `diagnose()`도 더는 이 코드를 알리지 않습니다.',
        lineAt: (line: number) => `${line}행`,
        retired: '폐기된 번호입니다. 이 코드를 단 진단은 없고, 레지스트리는 이 번호를 다른 규칙에 다시 주지 않습니다.',
        frontEnd: '같은 실수를 한 `"use typeshade"` 파일은 이 검사까지 오지 않습니다. 프런트엔드가 자기 코드로 먼저 거부하기 때문입니다. 빌드할 때 컴파일한 아래 프로그램에는 다음 진단이 나옵니다.',
        builder: '이 사이트의 `"use typeshade"` 프로그램 가운데 이 코드를 내는 것이 없어서 이 페이지에는 예제가 없습니다. [`fn()`](apiFn) 빌더는 IR을 직접 만들기 때문에 소스 절에 적힌 줄의 검사에 닿습니다.',
        internal: '레지스트리는 이 코드를 내부 불변식이라고 부릅니다. 이 코드가 나왔다면 컴파일러 자체가 잘못된 것이고, 어떤 프로그램도 이 코드를 내도록 되어 있지 않습니다.',
        unwritten: '이 커밋에는 아직 이 코드의 예제가 없습니다.',
        sourceP: (commit: string) => `커밋 ${commit}에서 컴파일러가 이 코드를 내는 곳입니다. 파일마다 한 줄씩 적었습니다.`,
        sites: (n: number) => `${n}곳`,
      ...
      lines: {
        MISSING_DIRECTIVE: 'TypeShade로 컴파일하는 파일이 `"use typeshade"` 지시문으로 시작하지 않습니다.',
        UNKNOWN_TYPE: '컴파일러가 모르는 타입 이름입니다.',
        TYPE_MISMATCH: '연산자나 선언, 반환, 인자처럼 두 값이 만나는 자리에서 타입이 서로 맞지 않습니다.',
        ...
        LOOP_INDUCTION: '`for` 루프의 카운터가 `i32`나 `u32` 타입의 `let` 하나가 아니거나, 갱신이 상수만큼 움직이지 않습니다.',
        BREAK_OUTSIDE: '감싸는 루프나 `switch`가 없는 곳에 `break`를 썼습니다.',
        STRUCT_FIELD: '구조체 리터럴이나 진입점의 입출력이 선언과 맞지 않습니다. 필드가 빠졌거나, 구조체에 없는 필드가 있거나, `@location`이 서로 어긋난 경우입니다.',
        ...
        TOP_LEVEL: '파일 최상위에 컴파일러가 선언으로 받을 수 없는 것이 있습니다. 선언이 아닌 문이거나, 거부하는 형태의 모듈 수준 선언입니다.',
        BACKEND: '프런트엔드가 받아들인 모듈을 백엔드가 출력하지 못하고 거부했습니다. 메시지는 백엔드가 쓴 그대로입니다.',
  ...
  playground: {
    ...
    gpuNone: '이 브라우저에는 WebGPU도 WebGL2도 없어서 아무것도 그리지 못했습니다. CPU 오라클은 그대로 돕니다.',
    ...
    gpuNeedsBindings: '바인딩 패널에 {names}에 넣을 값이 없어서 이 모듈은 여기서 실행할 수 없습니다.',
    computeNeedsWebgpu: '컴퓨트 진입점은 WebGPU나 CPU 오라클에서 실행됩니다. GLSL ES 3.00에는 컴퓨트 단계가 없습니다.',
    computeRan: '{entry}을(를) {backend}에서 호출 {invocations}회로 실행했고 {ms} ms 걸렸습니다. 캔버스는 이 진입점이 쓴 값을 그래프로 보여 줍니다.',
    cpuTextures: 'CPU 오라클에는 텍스처 유닛이 없어서, 여기서 텍스처를 읽으면 늘 검은색이 나옵니다.',
    bindings: {
      title: '바인딩',
      ...
      vertices: '버텍스 진입점은 이 입력을 버퍼에서 읽습니다. 페이지는 삼각형의 세 꼭짓점을 넘기고, 입력마다 이름에 맞는 값을 채웁니다.',
    },
    draw: 'CPU로 그리기',
    stop: '중지',
    ...
    emit: {
      ...
      levelNote: 'O0과 O1에서는 컴파일러가 레벨만 받아 WGSL을 생성하므로, 괄호와 최소화, 숫자 리터럴, 난독화, f64 에뮬레이션은 GLSL 탭에만 적용됩니다. f64 에뮬레이션은 `reflect()`가 따로 받으므로 모든 레벨에서 리플렉션에 반영됩니다.',
    },
  },
  ...
  concepts: {
    ...
    cpuAndGpu: {
      ...
      memoryRule: '여기에서 규칙이 나옵니다. `new`는 정체성을 가진 객체가 아니라 값을 만들고, 클래스 필드의 순서가 호스트가 쓰는 바이트의 레이아웃이 되며, 리소스는 모두 `declare`로 들어옵니다. 구조체 작성 인터페이스는 [타입](languageTypes)이, 선언은 [리소스](languageResources)가 적어 두었습니다.',
      callsH: '호출',
    ...
    pipeline: {
      ...
      interpolationNote: '실수 `@location` 필드는 `@interpolate`로 다른 방식을 지정하지 않는 한 이렇게 가중됩니다. 정수는 가중할 수 없으므로 컴파일러는 따로 적지 않아도 두 타깃 모두 `id`에 `flat` 한정자를 붙이고, 프래그먼트는 프리미티브의 버텍스 하나가 낸 값을 그대로 읽습니다. `tint`에 적은 `@interpolate("flat")`도 같은 처리를 요청합니다. 필드 데코레이터는 [타입](languageTypes)이 적어 두었습니다.',
      computeH: '컴퓨트',
  ...
  languageService: {
    ...
    packagingH: '패키지 구성',
    packagingP: '이 서비스는 `typescript` 위에서 돌아가며, 패키지는 이것을 필수 피어 의존성으로 둡니다. `compile()`이 `"use typeshade"` 파일을 TypeScript 파서로 읽기 때문에 기본 진입점에도 필요합니다. 그래서 컴파일러만 가져오는 프로그램도 TypeScript를 함께 설치하고, `typeshade/language-service`는 같은 패키지를 그대로 씁니다.',
    exampleH: '예제',
  ...
  examples: {
    ...
    shade: {
      ...
      descriptions: {
        ...
        'hello-vsout': '삼각형을 다시 그리되, 이번에는 버텍스 스테이지에서 프래그먼트 스테이지로 `uv` varying을 넘깁니다. 두 스테이지가 `VsOut` 클래스 하나를 함께 씁니다.',
        'hello-vsin': '`vertex_index` 대신 버퍼에서 버텍스 입력을 받습니다.',
        'bare-position': '가장 작은 렌더 쌍입니다.',
        'twin-structs': '필드가 똑같은 IO 구조체 둘, 곧 버텍스 출력과 프래그먼트 입력입니다. 객체 리터럴이 어느 쪽을 만드는지는 리터럴이 놓인 세 자리가 각각 정합니다.',
        'hello-uniform': '`declare const scale: uniform<f32>` 한 줄뿐입니다.',
        'hello-uniform-struct': 'GLSL ES 3.00 형태가 있는 쪽의 유니폼입니다.',
        'hello-camera': '`mat4`와 `vec3`를 담은 `Camera` 클래스를 `uniform<Camera>` 뒤에 두고, 평범한 헬퍼 함수가 읽습니다.',
        'textured-quad': '전체 화면 삼각형이 `sampler`를 거쳐 `texture_2d<f32>`를 샘플링하고, `override<f32>` 특수화 상수 두 개로 색을 입힙니다.',
        'array-length': '런타임 크기 스토리지 배열을 도는 커널이라면 반드시 필요한 경계 검사입니다.',
        'storage-texture': '컴퓨트 진입점이 텍셀 좌표로 직접 쓰는 이미지입니다. 샘플러도 필터링도 없습니다.',
        'shadow-compare': '`sampler_comparison`을 거쳐 깊이 텍스처를 `textureSampleCompare`와 `textureSampleCompareLevel`로 읽습니다. 평범한 2D 그림자 맵과 캐스케이드 배열 양쪽에서 씁니다.',
        'cube-env': '환경 맵은 방향으로 찾는 `texture_cube<f32>`, 컬러 그레이딩 표는 셰이딩한 색으로 색인하는 `texture_3d<f32>`입니다. `textureSampleBias`와 `textureSampleGrad`를 쓰고, 점광원의 그림자는 광원에서 나가는 방향으로 비교하는 `texture_depth_cube`입니다.',
        'cube-array-gather': '색상 램프는 `texture_1d<f32>`, 환경 맵 둘은 레이어로 고르는 `texture_cube_array<f32>`입니다. `textureGatherCompare`로 퍼센티지 클로저 필터를 손으로 짜고, `textureGather`로 텍셀 네 개에서 채널 하나를 모으며, 점광원의 그림자는 `texture_depth_cube_array`입니다.',
        'msaa-resolve': 'MSAA 렌더 타깃을 `texture_multisampled_2d<f32>`로 두고 `textureLoad(t, coords, sampleIndex)`로 샘플을 하나씩 읽어 `textureNumSamples`만큼 평균을 구합니다. 깊이 어태치먼트는 `texture_depth_multisampled_2d`입니다.',
        'module-const': '컴파일러가 허용하는 모든 스칼라 타입으로 모듈 범위 상수를 하나씩 선언합니다.',
        'palette-const': '전체 화면 삼각형을 모듈 범위의 `array<vec4, 3>` 팔레트와 `array<f32, 3>` 구간값으로 띠 지어 칠하고, 앞서 선언한 스칼라 상수로 만든 `vec3` 상수도 함께 씁니다.',
        'array-literal-ramp': '전체 화면 삼각형의 꼭짓점은 `array<f32, 3>` 목록 두 개에서 오고, 색은 `array<i32, 3>`으로 가중한 `array<vec3, 3>` 구간값에서 옵니다.',
        'convert-grid': '전체 화면 삼각형의 꼭짓점은 `vec2(vec2u(...))`에서 오고, 색은 `f32`에서 `u32`로 갔다가 다시 `f32`로 돌아오는 왕복에서 옵니다.',
        'normal-matrix': '`matCxR`는 모두 타입입니다.',
        'fp64-lane-stripes': '소스로 쓴 `f64` 인터페이스입니다.',
        'bitfield-bands': '전체 화면 삼각형의 색을 `switch`로 고릅니다. 띠 번호는 `&=`, `|=`, `<<=`, `>>=`, `^=`로 만들고, `let x: f32`는 값을 넣기 전에 먼저 선언하며, varying은 `{ pos, uv }` 축약형으로 돌려줍니다.',
        'block-scope': '`i`를 도는 반복문 둘이 이어지고, 반복문 본문의 `p` 옆에 `if` 분기의 `p`가 있으며, 안쪽 `p`가 바깥쪽을 가립니다.',
        'pick-composite': '분기 둘은 구조체, 다른 둘은 고정 길이 배열이고, 실행 중에 고릅니다.',
        cutout: '프래그먼트 진입점이 부르는 헬퍼 안에서 `discard`를 씁니다. `fwidth`가 테두리를 부드럽게 하고, `saturate`, `exp2`, `**`가 감쇠 곡선을 만듭니다.',
        'default-args': '매개변수에 기본값을 둔 헬퍼 셋입니다. 호출마다 생략하는 인자가 다릅니다.',
        'bit-bump': '`reflect`, `refract`, `faceForward`가 범프에 빛을 주고, `transpose`와 `determinant`가 호스트 행렬을 읽으며, 비트 내장 함수(`firstLeadingBit`, `reverseBits`, `countOneBits`, `extractBits`, `insertBits`)가 화면에 띠를 만듭니다. 띠가 시작하는 자리는 `fwidthCoarse`가 표시합니다.',
        'bool-select': '벡터 둘을 비교하면 결과는 불 벡터입니다.',
        'ray-class': '생성자와 메서드 하나, 정적 함수 하나를 둔 `class Ray`, 그리고 `hit(ray)` 메서드가 광선을 따라간 거리를 돌려주는 `class Sphere`입니다.',
        'orbit-inout': '`class Body`의 `step`, `turn`, `advance`가 `this`에 값을 넣으므로, 각 메서드는 자기 객체를 참조로 받습니다.',
        'particle-step': '`class Particle`의 `step`, `bounce`, `tick`이 `this`에 값을 넣고, 스토리지 요소 위에서 호출됩니다.',
        'shape-inheritance': '구체 메서드 하나와 추상 메서드 하나를 둔 `abstract class Shape`, 이를 상속한 클래스 둘, 그리고 그 하위 클래스를 다시 상속해 `super`를 부르는 클래스 하나입니다.',
        'mixin-surface': '본문이 `return class extends Base { ... }` 하나뿐인 `function Tinted(Base)`를 서로 다른 도형 클래스 둘에 적용합니다.',
        'generic-helpers': '제네릭 헬퍼 셋입니다. 파일이 부를 때 쓴 인자 타입 조합마다 한 번씩 컴파일됩니다.',
        'generic-class': 'f32와 vec3에 쓴 `class Slot<T>`입니다. 모듈에는 `Slot_f32`와 `Slot_vec3`가 서로 다른 구조체로 들어가고, 저마다 생성자와 모든 메서드의 복사본을 따로 갖습니다.',
        'tuple-and-brand': '튜플은 타입이 길이를 정해 둔 목록이고 `array<T, N>`이 바로 그것이므로, `[f32, f32]`는 곧 `array<f32, 2>`입니다.',
        'class-syntax': 'TypeScript 클래스를 쓰는 방식 그대로 작성한 `class Ring`입니다.',
        'rng-method': '`next()`가 생성기의 상태를 앞으로 옮기고 뽑은 값을 돌려주는 `class Rng`입니다.',
        'private-state': '최상위의 평범한 `let seed: u32`는 WGSL의 `var<private>`입니다. 인보케이션마다 복사본이 하나씩 있고, 그 인보케이션의 모든 함수가 함께 씁니다.',
        'workgroup-scratch': '`let tile: workgroup<array<f32, 64>>`는 WGSL의 `var<workgroup>`입니다. 워크그룹마다 복사본이 하나씩 있어 그 안의 인보케이션들이 함께 쓰며, 여기서는 인보케이션이 한 칸씩 차지하는 스크래치로 씁니다. 원자적 값의 워크그룹 배열과 인보케이션별 카운터도 함께 둡니다.',
        'workgroup-reduce': '인보케이션 64개가 값 64개를 워크그룹 메모리로 하나에 모읍니다. `workgroupBarrier()`가 라운드 순서를 잡습니다.',
        'atomic-histogram': '여러 인보케이션이 `atomicAdd(bins[bin], 1)`로 같은 칸에 동시에 값을 더합니다. 한 번의 호출은 쪼갤 수 없는 한 걸음이고, 스토리지 구조체의 필드와 `storage<atomic<u32>>` 선언이 위치를 적는 나머지 두 가지 모양을 보여 주며, 원자 연산이 돌려주는 값은 더하기 전에 들어 있던 값입니다.',
        ...
        'voronoi-twin': '`voronoi.ts`를 소스 언어로 다시 쓴 예제이며, 이슈 #40을 지키는 게이트입니다.',
        ...
        'clip-planes': '`@builtin("clip_distances")`로 사용자 클립 평면 네 개를 둡니다. 래스터라이저가 래스터화하기 전에 읽는 버텍스 출력입니다.',
        'uniform-array': '`array<f32, 4>`를 담은 `uniform`입니다.',
        'sample-branch': 'WGSL은 `textureSample`을 균일한 제어 흐름에서만 부르게 합니다.',
        'integer-math': '레지스트리가 이식 가능하다고 했지만 실제로는 그렇지 않았던 내장 함수 둘입니다.',
        'packing-bitcast': 'WGSL에는 있지만 이 표면에 없던 이식 가능한 내장 함수들입니다.',
        'packed-bytes': '`u32` 하나를 바이트 네 개로 읽거나 네 개를 다시 써 넣는 내장 함수 여덟 개입니다.',
  ...
  language: {
    ...
    topics: {
      types: {
        ...
        attrsP: '필드에는 파이프라인의 자리에 연결하는 `@location`과 `@builtin`을 붙이고, 스테이지 사이를 오가는 값이나 출력의 성질을 정하는 `@interpolate`, `@invariant`, `@blend_src`도 붙일 수 있습니다. `@align`은 읽고 나서 거부하며 `@size`, `@offset`, `@ignore`는 컴파일러가 아는 어트리뷰트가 아닙니다. 받아들이는 전체 집합은 `@vertex`, `@fragment`, `@compute`, `@builtin`, `@location`, `@interpolate`, `@invariant`, `@blend_src`, `@diagnostic`입니다.',
    ...
    mapping: {
      fromTypescript: {
        ...
        rows: {
          constScalar: { name: '스칼라 `const`', p: '모듈 상수입니다. 스칼라는 선언 자리에서 값 하나로 접힙니다. [module-const](shadeModuleConst)' },
          constVector: { name: '벡터 `const`', p: '벡터나 배열 상수는 값을 식으로 지니고, 백엔드가 그 식을 평가합니다. [palette-const](shadePaletteConst)' },
          letNoInit: { name: '지역 `let`', p: '변경 가능한 지역 변수입니다. 타입은 표기가 정하고, WGSL은 영으로 채워 둡니다. [bitfield-bands](shadeBitfieldBands)' },
          moduleLet: { name: '모듈 `let`', p: '모듈 변수입니다. 호출마다 하나씩 생깁니다. [private-state](shadePrivateState)' },
          varRefused: { name: '`var`', p: '호출별 변수는 `let`, 모듈 상수는 `const`입니다.' },
          enumRow: { name: '`enum`', p: '멤버마다 모듈 상수가 하나씩 생깁니다. 이름은 `Enum_Member`, 타입은 `i32`입니다.' },
          constEnum: { name: '`const enum`', p: '같은 상수가 생깁니다. 여기서 `const enum`은 다르지 않습니다.' },
          typeAlias: { name: '`type` 별칭', p: '대상을 가리키는 다른 이름입니다. 타입이 올 수 있는 자리마다 풀립니다.' },
          interfaceRow: { name: '`interface`', p: '구조체입니다. 같은 필드를 쓴 class와 결과가 같습니다.' },
          classStruct: { name: '구조체가 되는 클래스', p: '구조체입니다. 필드가 호스트의 메모리 레이아웃을 정합니다. [ray-class](shadeRayClass)' },
          namespaceRow: { name: '`namespace`', p: '멤버는 `Ns_member`로 펼쳐지고, namespace 자체는 아무것도 생성하지 않습니다.' },
          topFunction: { name: '최상위 함수', p: '이름과 매개변수와 반환 타입이 같은 모듈 함수가 됩니다.' },
          localFunction: { name: '지역 함수', p: '모듈의 함수가 됩니다. 이름은 이를 선언한 함수에서 따옵니다.' },
          noCapture: { name: '이름 캡처', p: '셰이더 함수에는 인자와 모듈만 있고, 이름을 담아 둘 환경이 없습니다.' },
          defaultArgs: { name: '기본 인자', p: '매개변수는 모두 남고, 빠뜨린 인자는 호출 지점에 적힙니다. [default-args](shadeDefaultArgs)' },
          overloads: { name: '오버로드 시그니처', p: '시그니처는 건너뛰고 구현 하나만 하향 변환합니다.' },
          recursion: { name: '재귀 호출', p: 'WGSL에는 호출 스택이 없고, 검사는 호출 그래프를 읽습니다.' },
          callStatement: { name: '문으로 쓴 호출', p: '문 하나가 됩니다. 값을 돌려주는 내장 함수가 홀로 서면 계산하는 것이 없어 최적화가 지웁니다.' },
          phonyAssign: { name: '효과가 있는 호출', p: '값을 돌려주면서 효과도 있는 내장 함수에는 WGSL의 `_ =` 대입이 붙습니다. Tint가 반환값을 반드시 쓰도록 요구하기 때문입니다. [atomic-histogram](shadeAtomicHistogram)' },
          mathAlias: { name: '`Math` 이름', p: '`Math.sin`은 `sin`과 같은 내장 함수이고, `Math.PI`는 값으로 접힙니다.' },
          constructorNew: { name: '`constructor`와 `new`', p: '`new`는 `Ray_new` 호출입니다. 구조체를 지어 돌려줍니다.' },
          method: { name: '메서드', p: '첫 매개변수가 구조체인 함수가 되고, `this`는 그 매개변수로 읽힙니다.' },
          staticFn: { name: '`static` 메서드', p: '수신자가 없는 함수가 되며, 클래스 이름을 앞에 답니다.' },
          thisAssign: { name: '`this`를 바꾸는 메서드', p: `객체를 바꾸는 메서드는 WGSL에서 포인터로, ${glsl}에서 \`inout\`으로 객체를 받습니다. [orbit-inout](shadeOrbitInout)` },
          extendsSuper: { name: '`extends`와 `super`', p: '기반 클래스의 필드가 앞에 오고, 물려받은 메서드는 다시 하향 변환됩니다. `super`는 자기 함수가 됩니다. [shape-inheritance](shadeShapeInheritance)' },
          abstractRow: { name: '`abstract` 클래스', p: 'abstract 클래스에는 구조체가 없습니다. 하위 클래스가 저마다 필드와 메서드 복사본을 가집니다.' },
          implementsRow: { name: '`implements`', p: 'TypeScript만 검사합니다. 구조체는 그 클래스 자신의 필드입니다.' },
          accessModifiers: { name: '접근 제어자', p: '받아들이지만 셰이더에는 아무 뜻이 없습니다. 강제하는 쪽은 TypeScript입니다.' },
          getter: { name: '게터와 세터', p: '각각 모듈의 함수 `Disc_get_area`와 `Disc_set_area`가 되고, 읽기와 쓰기가 그 함수를 부릅니다. [class-syntax](shadeClassSyntax)' },
          mixin: { name: '믹스인 함수', p: '함수는 컴파일하는 동안 실행됩니다. 멤버는 클래스에 끼워 넣고, `Tinted`는 어디에도 생성되지 않습니다. [mixin-surface](shadeMixinSurface)' },
          genericFunction: { name: '제네릭 함수', p: '파일이 쓰는 타입 인자 조합마다 함수가 하나씩 생기고, `pick`이라는 이름은 없습니다. [generic-helpers](shadeGenericHelpers)' },
          genericClass: { name: '제네릭 클래스', p: '타입 인자 조합마다 구조체가 하나씩 생기고, 메서드도 조합마다 따로 생깁니다. [generic-class](shadeGenericClass)' },
          ifRow: { name: '`if`', p: '적은 그대로 `if`입니다.' },
          forRow: { name: '고정 횟수 `for`', p: `횟수가 정해진 루프입니다. 정수 변수, 상수 경계, 상수 증가, 최대 ${facts.forTripLimit}회입니다. [block-scope](shadeBlockScope)` },
          forRefused: { name: '한도를 넘는 `for`', p: '반복 횟수가 루프에 허용된 한도를 넘습니다.' },
          whileRow: { name: '`while`', p: '컴파일러가 카운터를 붙인 루프가 됩니다. 본문이 경계로 다가가는지는 아무것도 검사하지 않습니다.' },
          switchRow: { name: '`switch`', p: '`switch`가 됩니다. TypeScript가 요구하는 `break`는 사라지고, 본문은 다음 case로 흘러가지 않습니다. [bitfield-bands](shadeBitfieldBands)' },
          ternaryScalar: { name: '스칼라 삼항 연산', p: `WGSL에서는 \`select\`가 되고, ${glsl}에서는 삼항 연산자가 됩니다.` },
          ternaryStruct: { name: '구조체 삼항 연산', p: '두 타깃 모두 이런 연산자가 없어서, 값을 슬롯 하나와 `if`로 끌어올립니다. [pick-composite](shadePickComposite)' },
          breakRow: { name: '`break`', p: '`break`입니다. 횟수가 정해진 루프를 일찍 빠져나오는 방법입니다. [julia-twin](shadeJuliaTwin)' },
          continueRow: { name: '`continue`', p: '`continue`입니다. 루프가 감싸지 않은 `switch` 안에서는 거절합니다.' },
          discardRow: { name: '`discard`', p: '`discard`입니다. 프래그먼트 진입점이나 그 진입점이 부르는 함수에 씁니다. [cutout](shadeCutout)' },
          destructuring: { name: '구조 분해', p: '이름마다 선언이 하나씩, 적힌 순서대로 풀립니다.' },
          spread: { name: '객체 스프레드', p: '구조체의 필드마다 읽기가 하나씩 생기고, 뒤에 적은 필드가 그 위를 덮습니다.' },
          arrayLiteral: { name: '배열 리터럴', p: '배열 초기자입니다. 선언이 `array<T, N>`을 밝힌 자리에서만 씁니다. [array-literal-ramp](shadeArrayLiteralRamp)' },
          tuple: { name: '튜플', p: '튜플은 타입이 길이를 고정한 배열입니다. 반환 자리에서도 그렇습니다. [tuple-and-brand](shadeTupleAndBrand)' },
          literalUnion: { name: '리터럴 유니온', p: '멤버가 모두 한 타입을 가리키는 유니온은 그 타입을 가리킵니다.' },
          brand: { name: '브랜드 타입', p: '브랜드는 지워지고 매개변수는 `f32`입니다. [tuple-and-brand](shadeTupleAndBrand)' },
          typeClaims: { name: '타입 주장', p: '타입에 대한 주장일 뿐 변환이 아니라서, 피연산자가 생성하는 코드를 그대로 생성합니다.' },
          power: { name: '거듭제곱 연산자', p: '두 타깃 모두 `pow`입니다.' },
          logicalScalar: { name: '`bool`에 쓰는 `&&`', p: '두 타깃이 모두 가진 연산자이며, 한 번에 `bool` 하나를 다룹니다.' },
          logicalVector: { name: '마스크에 쓰는 `&&`', p: '마스크는 `all`이나 `any`나 `select`로 묶으십시오. [bool-select](shadeBoolSelect)' },
          optionalMember: { name: '선택 필드', p: '구조체 필드는 호스트가 채우는 메모리에 언제나 있습니다.' },
          stringValue: { name: '문자열 값', p: 'GPU에 문자열은 없습니다. 경우를 enum으로 적으십시오.' },
          numberType: { name: '`number`', p: 'GPU의 수에는 너비가 있습니다.' },
          booleanType: { name: '`boolean`', p: '셰이더 표기는 `bool`입니다.' },
          integerLiteral: { name: '정수 리터럴', p: '리터럴은 그 자리가 밝힌 타입을 따르고, 그 타입으로 접힙니다.' },
          increment: { name: '증감 연산자', p: '값이 한 걸음 옮겨 간 결과를 대입합니다.' },
          f64Scalar: { name: '`f64` 스칼라', p: '`f32` 두 워드의 쌍이 되고, 연산마다 `df64_` 호출이 붙습니다. [fp64-lane-stripes](shadeLaneStripes)' },
          f64Literal: { name: '`f64` 리터럴', p: '`f64`라고 밝힌 자리의 리터럴은 배정밀도 값을 그대로 지닙니다.' },
          f64Vector: { name: '`f64` 벡터', p: '배정밀도 벡터입니다. hi 평면과 lo 평면으로 하향 변환됩니다. `vec2d`는 `vec2f64`의 짧은 표기입니다.' },
          f64Builtin: { name: '`f64` 내장 함수', p: '스칼라에서 내장 함수 열 개, 벡터에서 열세 개가 에뮬레이션 본체를 가집니다.' },
          f64Refused: { name: '`f64` 본체가 없는 내장 함수', p: '호출 자리에서 거절하며, 그 열 개와 좁히는 방법을 함께 알려 줍니다.' },
          f64Guard: { name: '유니폼 안의 `f64`', p: '`_fp64` 텍스처가 주입됩니다. 호스트가 `1.0`으로 채우고, 리플렉션이 다른 바인딩처럼 보여 줍니다.' },
          f64Varying: { name: '스테이지 사이의 `f64`', p: '배정밀도 값은 필요한 스테이지에서 읽거나, 경계에서 `f32`로 좁히십시오.' }
        },
      ...
      fromWgsl: {
        ...
        h1: 'WGSL을 TypeShade로',
        intro: '이미 WGSL을 쓰는 사람이 이 사이트에서 찾는 것은 하나입니다. 눈앞에 있는 구성 요소를 TypeShade로 어떻게 적느냐입니다. 이 페이지는 그 방향으로 읽힙니다. 왼쪽에 WGSL 형태, 오른쪽에 `"use typeshade"` 파일에 적을 코드, 그리고 둘이 맞아떨어지지 않는 자리에는 설명을 답니다.',
        readingH: '표 읽는 법',
        ...
        vectorsH: '벡터',
        vectorsP: '`vec2`와 `vec4`도 위의 세 가지와 같은 방식으로 읽습니다. `vec3`와 `vec3f`는 같은 타입이고, `vec3d`와 `vec3f64`도 마찬가지입니다.',
        matricesH: '행렬',
        matricesP: '정사각 행렬에는 두 타깃이 모두 주는 짧은 이름도 있어서 `mat4`와 `mat4x4`는 같은 선언입니다. `C`와 `R`의 뜻도 WGSL과 같아서 앞이 열, 뒤가 행입니다.',
        arraysH: '배열과 atomic',
        arraysP: '튜플은 타입이 길이를 고정한 목록이고, 크기를 아는 배열이 바로 그것입니다. 그래서 `[f32, f32]`와 `array<f32, 2>`는 같은 타입에 닿습니다. 길이가 실행 시점에 정해지는 배열과 atomic은 각각 스토리지 바인딩 안에 놓입니다.',
        texturesH: '텍스처와 샘플러',
        texturesP: '텍스처와 샘플러는 주소 공간 래퍼 없이 그냥 적습니다. 핸들은 어느 주소 공간에도 놓이지 않기 때문입니다. 샘플링 텍스처는 `f32`나 `i32`나 `u32`를 받고, 그 요소 타입이 WGSL 표기와 적용 가능한 읽기 방식을 함께 결정합니다. 스토리지 텍스처는 포맷과 접근 모드를 문자열 리터럴 타입으로 받으므로 이 컴파일러보다 `tsc`가 먼저 검사합니다.',
        resourcesH: '리소스와 주소 공간',
        resourcesP: '리소스는 `declare`로 적습니다. 다른 쪽이 제공하는 값을 가리키는 TypeScript의 표현이 그것입니다. 주소 공간과 접근 모드는 타입 주석의 래퍼 타입이 나타내고, 나머지는 `const`와 `let`의 차이가 맡습니다. 셰이더가 쓰는 `storage` 바인딩은 `declare let`입니다.',
        slotsP: '`@group`이나 `@binding`은 적지 않습니다. 슬롯 번호는 파일 안 `declare`의 등장 순서이고, 위의 WGSL이 그 결과입니다. 텍스처와 샘플러도 같은 방식으로 다음 슬롯을 가져갑니다.',
        stagesH: '진입점과 속성',
        stagesP: '스테이지 속성은 export한 함수에 붙이는 데코레이터입니다. 셋 중 아무것도 붙지 않은 함수는 헬퍼입니다. `@compute`가 받는 값은 워크그룹 크기 하나뿐이고 목록으로 적습니다. 컴파일러는 첫 번째 숫자만 나르며, y나 z가 1이 아닌 형태는 거부합니다.',
        returnsP: '버텍스 진입점은 반드시 위치를 만들어 내야 합니다. 반환 타입이 `vec4`이면 그 자체가 position builtin을 답니다. 구조체를 반환하면 위치는 필드 하나가 나르고, 프로그램이 원하는 만큼 `@location` varying을 덧붙입니다. position 필드가 없는 구조체 반환, `void` 반환, `vec4`가 아닌 단일 타입은 각각 거부되며 어떤 필드나 타입을 적어야 하는지 함께 알려 줍니다.',
        ...
        absentH: 'WGSL에 없는 이름',
        absentP: 'GLSL 작성자가 손이 가는 이름 셋에는 대응하는 WGSL builtin이 없습니다. 프런트엔드는 선언 자리에서 각각을 거부하고 받아들이는 이름들을 함께 보여 줍니다. 오른쪽 칸은 백엔드가 속성 이름 뒤에 이어 붙이는 문장이고, 대신 무엇을 쓰면 되는지 말해 줍니다.',
        statementsH: '문과 식',
        statementsP: '문은 TypeScript의 것이고, 각각이 옆의 WGSL과 같은 뜻입니다. 생성된 텍스트는 빌드 시점에 컴파일한 파일 하나에서 가져왔으므로 괄호와 접힌 상수도 생성기가 쓴 그대로입니다.',
        functionsH: '내장 함수',
        functionsP: '컴파일러가 표기할 수 있는 내장 함수와 각각에 대해 생성하는 WGSL 텍스트는 [내장 함수 표](languageBuiltins)에 있습니다. 파일이 직접 선언한 함수는 같은 이름의 내장 함수보다 우선합니다.',
        refusals: {
          'scalars.f16': '이 인터페이스에는 타입 이름이 없습니다. `"use typeshade"` 옆에 `"enable f16"`을 적으면 WGSL의 `enable f16;`이 생성되고, 리플렉션에는 호스트가 `shader-f16`으로 요청할 기능이 실립니다. 그래도 값을 선언할 `f16`은 아직 없습니다.',
          'scalars.f64': 'WGSL에는 64비트 부동소수점이 없습니다. 아래 패스가 생성기보다 먼저 선언을 바꿔 놓습니다.',
          'vectors.vec3d': 'WGSL에는 64비트 벡터도 없어서 같은 패스가 이것도 바꿔 놓습니다.',
        },
        notes: {
          'stages.vertex': '진입점 하나에 스테이지 데코레이터 하나를 export한 함수에 붙입니다.',
          'stages.fragment': '프래그먼트 진입점은 아무것도 반환하지 않아도 됩니다. 스토리지 바인딩에만 쓰는 프로그램이 그런 모양입니다.',
          'stages.compute': '목록이 워크그룹 크기입니다. 목록 없는 `@compute`는 `[64, 1, 1]`입니다.',
          'stages.location': 'location은 진입점 매개변수와 클래스 필드와 반환 자리에 붙습니다.',
          'stages.vertexBare': '반환 타입이 `vec4`이면 그 자체가 position builtin을 답니다. 그래서 가장 작은 버텍스 셰이더에는 구조체도 매개변수도 필요 없습니다.',
          'stages.vertexStruct': '구조체를 반환하면 위치는 필드 하나가 나르고, 그 옆에 프로그램이 원하는 만큼 varying을 덧붙입니다.',
          'stages.fragmentReturn': '프래그먼트가 단일 값을 반환하면 너비와 상관없이 location 0을 가져갑니다. 그래서 `f32`, `vec2`, `vec3`, `vec4`가 모두 드로 버퍼 포맷이 됩니다. 구조체 반환은 렌더 타깃이 여럿일 때 쓰는 형태입니다.',
          'statements.let': 'WGSL의 `let`은 그대로 있는 값이고, TypeScript에서 그 뜻을 말하는 것이 `const`입니다.',
          'statements.var': 'WGSL의 `var`는 바뀌는 지역 변수입니다. 초기값 없는 `let b: f32`가 그것을 선언하고 값은 나중 대입에 맡깁니다. 첫 대입 전에 읽으면 WGSL은 영으로 채운 값을 주고 GLSL은 정해지지 않은 값을 주므로, 읽기 전에 대입하십시오.',
          'statements.for': '`for`는 횟수가 세어지는 형태여야 합니다. 정수 유도 변수, 상수 경계, 상수 증감, 그리고 최대 256회입니다. 증감에는 `+=`, `-=`, `*=`, `/=`를 쓸 수 있습니다.',
          'statements.while': '`while`이 `loop` 자리를 대신합니다. 조건에 컴파일 시점 상수 경계가 있어야 하고, 타깃에는 컴파일러가 덧붙인 카운터를 도는 `for`로 도착합니다.',
          'statements.switch': 'case는 TypeScript가 요구하는 `break`로 끝내고, 하향 변환이 그것을 떼어 냅니다. 어느 case도 다음으로 흘러가지 않으며, 레이블은 한 번만 쓸 수 있는 정수 상수입니다. 본문 하나 위에 레이블 두 개를 겹쳐 쓰면 선택자가 둘인 case 하나가 되며, WGSL에서는 `case 0, 1:`로 나옵니다.',
          'statements.select': 'WGSL에는 삼항 연산자가 없어서 스칼라와 벡터 조건식은 `select`가 됩니다. 첫 인자가 조건이 고르지 않은 쪽입니다. 구조체나 배열을 고르는 조건식은 슬롯과 `if`로 풀립니다. 그 경우에는 어느 타깃에도 연산자가 없기 때문입니다.',
          'statements.call': '결과를 버리고 동작만 보고 부르는 함수입니다.',
          'statements.phony': '같은 방식으로 결과를 버린 내장 함수 호출에는 가짜 대입이 붙습니다. Tint가 그런 내장 함수를 모두 `@must_use`로 읽기 때문입니다.',
          'statements.discard': '같은 낱말이고 같은 문입니다.',
          'statements.struct': 'WGSL 구조체는 클래스입니다. 객체 모양의 타입 별칭과 인터페이스도 같은 선언에 닿습니다.',
      ...
      fromGlsl: {
        ...
        scalarsH: '스칼라',
        vectorsH: '벡터',
        vectorsP: '`vec2`와 `vec4`도 같은 방식으로 읽고, 각각의 정수형과 부호 없는 정수형과 불리언 형태도 마찬가지입니다. 배정밀도 벡터는 어느 타깃에도 형태가 없어서 생성기가 돌기 전에 `f32` 레인으로 하향 변환됩니다.',
        arraysH: '행렬과 배열',
        matricesH: '행렬',
        matricesP: '정사각 행렬은 드라이버 오류 메시지가 쓰는 짧은 이름을 그대로 씁니다. 나머지 여섯 가지 모양은 이 타깃이 적는 방식과 똑같이 `matCxR`로, 앞이 열이고 뒤가 행입니다.',
        arraysP: '튜플은 타입이 길이를 고정한 목록이므로 `[f32, f32]`와 `array<f32, 2>`는 같은 타입에 닿습니다. 실행해 봐야 길이를 아는 배열은 스토리지 버퍼이고 이 타깃에는 그 형태가 없어서, 그런 모듈은 데이터 텍스처에서 읽도록 다시 쓰입니다.',
        samplersH: '샘플러',
        samplersP: '이 타깃은 텍스처와 샘플러를 결합 샘플러 하나로 합치고, WebGPU는 둘을 따로 둡니다. TypeShade 파일은 둘을 WebGPU 방식으로 선언하고 GLSL 생성기가 합치므로, 샘플러 인자는 모든 읽기에서 사라지고 그 자리를 차지하던 바인딩도 호스트가 바인딩하는 레이아웃에서 빠집니다.',
        shadowP: '깊이 텍스처는 그 합치기의 예외입니다. 결합 타입이 모듈이 그것을 어떻게 읽느냐에 달려 있어서 바인딩 생성 단계가 결정합니다. 이 인터페이스가 허용하는 깊이 텍스처 읽기는 모두 비교이고, 그래서 항상 shadow 형태가 됩니다.',
        noFormH: 'GLSL 형태가 없는 타입',
        noFormP: '아래 타입은 모두 생성기가 거부하며, 옆 칸은 생성기가 던지는 문장 그대로입니다. 그런 타입을 든 모듈도 WGSL은 생성하므로, 셰이더를 WebGPU 전용으로 만드는 줄이 바로 이것들입니다.',
        uniformsH: '유니폼과 버퍼',
        uniformsP: '유니폼 바인딩은 구조체이고, 생성기는 이것을 std140 블록으로 생성합니다. 스토리지 버퍼는 이 타깃에 아예 형태가 없어서, 그런 모듈은 기능 게이트가 돌기 전에 데이터 텍스처에서 읽도록 다시 쓰입니다.',
        varyingsH: 'varying',
        varyingsP: '버텍스 스테이지에서 프래그먼트 스테이지로 넘어가는 값은 양쪽 모두에서 `@location(n)` 필드입니다. 이 타깃은 그 둘을 이름으로 잇고 WebGPU는 번호로 이으므로, 생성된 GLSL에서 맞아야 하는 것은 필드 이름입니다.',
        ...
        enablesP: '`"use typeshade"` 파일에는 이 네 가지를 적을 방법이 없습니다. `"enable ..."`은 WGSL 확장 이름을 받는데 넷 중 어느 것도 WGSL 확장이 아니므로, 이런 기능이 필요한 모듈은 `fn()` 인터페이스의 `module({ enables: [...] })`로 조립합니다. 스토리지 바인딩, 컴퓨트 진입점, `@builtin("clip_distances")`처럼 모듈의 모양에서 따라오는 기능은 파일에서 도출되며, 그중 어느 것도 이 표에 행이 없습니다.',
        variablesH: 'builtin 변수',
        variablesP: '`gl_*` 전역 변수는 매개변수나 클래스 필드나 반환 자리에 붙는 `@builtin(...)` 속성입니다. 어떤 값인지는 속성이 말하고 그것이 어느 전역 변수가 될지는 생성기가 정합니다. 파일 하나가 두 타깃에 모두 닿는 방식이 이것입니다.',
        absentH: '어느 생성기도 받지 않는 변수',
        absentP: '둘은 이 인터페이스에 이름이 없습니다. 프런트엔드가 선언 자리에서 그 표기를 거부하며, 아래 문장은 같은 이름에 대해 WGSL 백엔드가 내는 말이고 대신 무엇을 쓰면 되는지 알려 줍니다.',
        functionsH: '함수와 연산자',
        functionsP: '호출은 중립 이름 하나로 컴파일러를 통과하고 백엔드마다 자기 표기를 씁니다. 그래서 적어야 할 이름은 오른쪽 칸의 것입니다. 아래는 GLSL 작성자가 특히 많이 찾는 이름들입니다.',
        operatorsP: '연산자도 같은 방식으로 갈립니다. 벡터 두 개를 비교할 때는 연산자로 적고 이 타깃에는 함수로 도착하며, 성분마다 고르는 연산은 양쪽 모두 `select`입니다.',
        operatorsH: '연산자',
        restP: '컴파일러가 표기할 수 있는 내장 함수와 두 타깃에서 각각 생성하는 텍스트는 [내장 함수 표](languageBuiltins)에 있습니다.',
        edslH: 'fn() 표기',
        edslP: '이 페이지가 다루는 것은 `"use typeshade"` 인터페이스입니다. 컴파일러에는 `fn()`과 `uniformStruct()` 같은 함수로 이루어진 내장 인터페이스도 있고, 같은 GLSL 구성 요소를 그 표기와 나란히 놓은 글이 [GLSL 셰이더 옮기기](internalsGlslShader)입니다. 이 페이지의 WGSL 쪽은 [WGSL을 TypeShade로](languageFromWgsl)입니다.',
        refusals: {
          'uniforms.precision': '적을 것이 없습니다.',
        },
        notes: {
          'uniforms.block': '클래스가 블록이고 `declare`가 그 인스턴스입니다. 필드 순서가 곧 레이아웃입니다.',
          'uniforms.loose': '`glUniform*`로 값을 넣는 기본 블록 유니폼은 여기에 표기가 없습니다. 그래서 스칼라나 벡터나 행렬 하나를 담은 유니폼 바인딩은 WebGPU에만 닿습니다.',
          'uniforms.storage': '버퍼가 데이터 텍스처가 되고, 호스트는 같은 숫자를 텍셀로 올립니다.',
          'uniforms.fetch': '버퍼를 인덱스로 읽는 자리가 이 함수 호출이 됩니다. 생성기는 이 함수를 부르는 모듈에만 정의를 넣습니다.',
          'uniforms.define': '특수화 상수에는 GLSL 형태가 없어서, 기본값을 전처리기 치환으로 적어 둡니다. 호스트는 스테이지를 컴파일하기 전에 그 값을 덮어쓸 수 있습니다.',
          'uniforms.precision': '생성기가 스테이지마다 맨 위에 한정자를 넣고, `floatPrecision` 옵션이 float 줄을 `highp`로 할지 `mediump`로 할지 정합니다. 정수 줄은 `highp`로 둡니다. 데이터 텍스처를 거쳐 되읽는 값에는 정수 범위가 온전히 필요하기 때문입니다.',
          'varyings.out': '버텍스 진입점이 반환하는 구조체의 필드에 적습니다.',
          'varyings.in': '그 값을 읽는 프래그먼트 진입점 매개변수에 같은 이름으로 적습니다.',
          'varyings.target': '프래그먼트가 단일 값을 반환하면 location 0을 가져가고, 출력 이름은 생성기가 직접 붙입니다.',
          'variables.glPosition': '버텍스 출력에 붙는 같은 id입니다. `vec4` 하나를 반환하는 버텍스 진입점은 아무것도 적지 않아도 이것을 답니다.',
          'variables.glFragCoord': '프래그먼트 입력에 붙는 같은 id입니다. y 원점이 다릅니다. 이 타깃은 창 아래쪽부터 세고 WebGPU는 위쪽부터 세므로, `.y`를 읽는 셰이더는 스스로 뒤집어야 합니다.',
          'variables.glVertexID': '인덱스가 여기서는 `u32`이고 저기서는 `int`이라, 읽기를 선언한 타입으로 감쌉니다.',
          'variables.glInstanceID': '같은 이유로 같은 방식으로 감쌉니다.',
          'variables.glFrontFacing': '양쪽 모두 `bool`이라 읽기에 캐스트가 붙지 않습니다.',
          'variables.glFragDepth': '프래그먼트 진입점이 반환하는 구조체의 필드로, 색상 옆에 적습니다.',
          'functions.dpdx': 'WGSL 이름으로 부르는 같은 미분입니다.',
          'functions.dpdy': '축만 다른 같은 미분입니다.',
          'functions.dpdxCoarse': 'coarse와 fine 변형은 WGSL에서는 이름을 유지하고 여기서는 하나로 합쳐집니다. 이 타깃에는 축마다 미분이 하나뿐이기 때문입니다.',
          'functions.dpdyFine': '같은 방식으로 합쳐집니다.',
          'functions.mod': '나누는 수 쪽으로 내림한 나머지이고, 이 타깃의 `mod`가 이미 그것입니다. `%` 연산자는 다른 쪽입니다.',
          'functions.textureSample': '결합 샘플러가 이미 샘플러를 들고 있으므로 샘플러 인자가 사라집니다.',
          'functions.textureLoad': '정수 좌표와 밉 레벨로 텍셀 하나를 읽습니다.',
          'functions.textureDimensions': '크기가 부호 없는 값으로 돌아오므로, 생성기가 이 타깃의 부호 있는 답을 감쌉니다.',
          'functions.faceForward': '표기만 다릅니다.',
          'functions.inverseSqrt': '이것도 표기만 다릅니다.',
          'functions.round': '두 타깃이 함께 쓰는 반올림은 절반을 짝수 쪽으로 보내는 방식입니다.',
          'functions.countOneBits': '이 타깃에는 비트 내장 함수가 하나도 없어서, 생성기가 여기에 있는 시프트와 마스크로 작은 함수를 정의합니다.',
          'functions.firstLeadingBit': '그런 함수가 하나 더 있고, 부르는 모듈에만 정의가 들어갑니다.',
          'operators.mod': '나누는 수 쪽으로 내림한 나머지라 부호가 나누는 수를 따릅니다.',
          'operators.remainder': '잘라 내는 나머지라 부호가 나뉘는 수를 따릅니다. 이 타깃에는 실수에 쓰는 `%`가 없어서 생성기가 식을 풀어서 적습니다.',
          'operators.lessThan': '벡터 비교는 이 타깃에서는 함수이고 저쪽에서는 연산자입니다. 그래서 소스에는 연산자로 적습니다.',
          'operators.equal': '같은 방식이고, TypeScript가 받는 것은 엄격 비교 쪽입니다.',
          'operators.select': '스칼라 선택은 여기서는 삼항 연산자이고 WGSL에서는 `select`라, 이름 하나가 양쪽을 맡습니다.',
          'operators.vectorSelect': '불리언 벡터로 성분마다 고르는 연산은 이 타깃에서 `mix`이고, GLSL 작성자가 이미 쓰던 모양이 그것입니다.',
        }
      },
      builtins: {
