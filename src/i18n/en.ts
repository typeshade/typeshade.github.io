...
export const en = {
  ...
  docs: {
    ...
    errors: {
      ...
      verified: (n: number) => `${n} pages carry a program that raises the code and the same program fixed. Both are compiled at build time against the pinned compiler, and the build stops when a program no longer raises its code or a fix no longer compiles clean. A row with an Example tag is one of them.`,
      ...
      frontEndP: 'The `TS80xx` codes, from `TS_CODES` in the front end. A `"use typeshade"` file is checked against these before any IR is built.',
      coreH: 'Core codes',
      coreP: 'The `SDxxxx` codes, from the `CODES` catalogue in `typeshade/dev`. The core IR layer raises them under the [`fn()`](apiFn) builder and under a `"use typeshade"` file alike, and the front end refuses many of the same mistakes first under a code of its own.',
      ...
      entry: {
        ...
        title: (head: string, summary: string) => {
          const candidates = [
            ...(summary ? [`${head}: ${summary}, a TypeShade error code`, `${head}: ${summary} in TypeShade`, `${head}: ${summary}`] : []),
            `${head}: an error code of the TypeShade compiler`,
            `${head}: a TypeShade compiler error code`,
            `${head}: TypeShade error code`,
          ]
          return candidates.find((t) => t.length >= 45 && t.length <= 60) ?? candidates[candidates.length - 1]!
        },
        description: (head: string, line: string) => `${head}, an error code of the TypeShade compiler. ${line}`,
        ...
        kindMeta: 'Kind',
        undocumented: 'The registry has no documentation comment for this constant, so the line above is written by this site from the messages the compiler raises it with.',
        whenH: 'When it fires',
        exampleH: 'Example',
        fixH: 'How to fix it',
        ...
        backend: 'Compiled at build time against the pinned compiler, this program gets a `TS8015` diagnostic, the front end reporting a backend that refused the module. The error the backend threw carries the code:',
        diagnose: '`compile()` does not run the lint rules. `diagnose()` from `typeshade/dev` does, on the module `compile()` returns, and reports:',
        deprecations: 'Compiled with `{ deprecations: true }`, the option that turns this warning on.',
        hint: "The registry's hint:",
        fixed: 'The same program, corrected. It compiles with no diagnostic.',
        fixedDiagnose: 'The same program, corrected. It compiles with no diagnostic, and `diagnose()` no longer reports the code.',
        lineAt: (line: number) => `line ${line}`,
        retired: 'This number is retired. No diagnostic carries it, and the registry never gives it to another rule.',
        frontEnd: 'A `"use typeshade"` file never reaches this check with the same mistake: the front end refuses it first, under its own code. This program, compiled at build time, gets:',
        builder: 'No `"use typeshade"` program on this site raises this code, so the page has no example. The [`fn()`](apiFn) builder builds the IR directly and reaches the check at the lines under Source.',
        internal: 'The registry calls this an internal invariant: it means the compiler itself went wrong, and no program is meant to raise it.',
        unwritten: 'No example is written for this code at this commit yet.',
        sourceP: (commit: string) => `Where the compiler raises this code at commit ${commit}, one line per file:`,
      ...
      lines: {
        MISSING_DIRECTIVE: 'A file compiled as TypeShade that does not start with the `"use typeshade"` directive.',
        UNKNOWN_TYPE: 'A type name the compiler does not know.',
        TYPE_MISMATCH: 'Two types that do not fit where they meet: an operator, a declaration, a return or an argument.',
        UNKNOWN_FN: 'A call to a function the file neither declares nor imports.',
        CONST_ASSIGN: 'An assignment to a name that cannot change, such as a `const` or a read-only resource.',
        LOOP_BOUND: 'A counted `for` loop whose exit does not compare its counter to a constant bound.',
        LOOP_INFINITE: 'A loop whose condition is always true.',
        LOOP_INDUCTION: 'A `for` loop whose counter is not one `let` of type `i32` or `u32`, or whose update is not a constant step.',
        BREAK_OUTSIDE: 'A `break` with no loop or `switch` around it.',
        STRUCT_FIELD: "A struct literal or an entry's IO that does not match its declaration: a field missing, a field the struct does not have, or a `@location` that does not line up.",
        HOST_API: 'A JavaScript host API, such as `window` or `fetch`, named inside a shader file.',
        HOST_STMT: 'A JavaScript statement a shader has no form for, such as `try`, `throw` or `await`.',
        TOP_LEVEL: 'Something at the top level of the file the compiler cannot declare: a bare statement, or a module-level declaration of a shape it refuses.',
        BACKEND: "A backend that refused to emit a module the front end accepted. The message is the backend's own.",
