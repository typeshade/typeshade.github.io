...
const summary = [
  ...
  `zero runtime dependencies, is ${facts.license} licensed, and its own CI compiles every WGSL`,
  `emit on Tint and links every renderable example on WebGL2, on every push. Version`,
  `${facts.nextVersion} is not published yet and the npm name typeshade is reserved, so start`,
  `from the read-only mirror and the authoring guide.`,
].join(" ");
...
const table = [
  ...
  {
    dest: links.mirror,
    note: "read-only mirror; consume as a git submodule, its root is the package",
  },
  { dest: links.docs, note: "the mirror's README" },
  { dest: links.npm, note: "" },
...
const strays = [...new Set(prose.match(NUMERAL) ?? [])].filter(
  (n) => !known.has(n),
);
if (strays.length > 0) {
  throw new Error(
