...
export interface ComputeResult {
  ...
  readonly textures: ReadonlyMap<string, { readonly width: number; readonly height: number; readonly format: string; readonly bytes: Uint8Array }>
  readonly ms: number
}
...
export async function runComputeOnGpu(job: ComputeJob): Promise<ComputeResult> {
  ...
  try {
    ...
    for (const [group, list] of layouts) groupLayouts.set(group, device.createBindGroupLayout({ entries: list }))
    const empty = device.createBindGroupLayout({ entries: [] })
    const top = Math.max(-1, ...groupLayouts.keys())
    const constants = job.constants && Object.keys(job.constants).length > 0 ? { ...job.constants } : undefined
  ...
  const reads: { name: string; staging: GPUBuffer; texture?: { width: number; height: number; format: string; padded: number; row: number } }[] = []
  for (const r of job.resources) {
    ...
    if (r.kind === 'storage-texture' && r.access !== 'read-only') {
      ...
      const staging = device.createBuffer({ size: padded * r.height, usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST })
      const texture = textureOf.get(r.name)
      if (!texture) continue
      encoder.copyTextureToBuffer({ texture }, { buffer: staging, bytesPerRow: padded }, [r.width, r.height])
      reads.push({ name: r.name, staging, texture: { width: r.width, height: r.height, format: r.format, padded, row } })
    }
  }
  ...
  const textures = new Map<string, { width: number; height: number; format: string; bytes: Uint8Array }>()
  for (const r of reads) {
    ...
    for (let y = 0; y < height; y++) tight.set(bytes.subarray(y * padded, y * padded + row), y * row)
