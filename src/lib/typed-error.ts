...
function region(text: string, name: string): { body: string; firstLine: number } {
  ...
  if (start < 0 || end < 0 || end <= start + 1)
    throw new Error(
      `[typed-error] ${FIXTURE} has no '// #region ${name}' … '// #endregion ${name}' pair`,
    );
...
function checkBothArms(
  ...
  if (!wrongFile || !fixedFile)
    throw new Error('[typed-error] the in-memory arms did not reach the program');
...
export function typedError(): TypedError {
  ...
  if (snippetLines > MAX_SNIPPET_LINES)
    throw new Error(
      `[typed-error] snippet is ${snippetLines} lines; the brief caps it at ${MAX_SNIPPET_LINES}`,
    );
  ...
  if (wrongSource === fixtureSource)
    throw new Error('[typed-error] the snippet region did not substitute');
  ...
  if (wrong.length === 0)
    throw new Error(`[typed-error] the wrong arm ('${WRONG_READ}') produced no diagnostic`);

  const located = wrong.find((d) => d.file === wrongFile && d.start !== undefined);
  if (!located)
    throw new Error(
      `[typed-error] no diagnostic landed in the snippet: ${wrong.map(describe).join(' | ')}`,
    );
  ...
  if (wrongLine < 1 || wrongLine > snippetLines)
    throw new Error(
      `[typed-error] the diagnostic landed on line ${line + 1}, outside the snippet region`,
    );
