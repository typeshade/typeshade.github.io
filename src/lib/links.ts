...
export const links = {
  ...
  fromTsDeclarations: { label: 'Declarations', href: '/guide/language/from-typescript/declarations/' },
  ...
  fromTsControlFlow: { label: 'Control flow', href: '/guide/language/from-typescript/control-flow/' },
...
export function mappingSet<K extends MappingIndexKey>(index: K): Extract<(typeof mappingSets)[number], { index: K }> {
...
type DocsPageKey =
  | 'motivation' | 'quickStart' | 'playground' | 'guide'
  | 'languageFromTypescript' | 'languageFromWgsl' | 'languageFromGlsl' | 'languageBuiltins'
  | MappingSectionKey
  | 'concepts' | 'conceptsCpuGpu' | 'conceptsPipeline' | 'conceptsWebgpu' | 'conceptsWgsl'
  | 'examples' | 'reference' | 'internals' | 'languageService' | 'checks'
