...
function nameIn(text: string, name: string, where: string): string {
  if (!text.includes(name)) throw new Error(`[glsl-mapping] the ${where} no longer writes '${name}'`)
  return name
}
...
function build(): GlslMapping {
  ...
  if (!looseMessage) {
    throw new Error('[glsl-mapping] a bare scalar uniform no longer names the std140 block it wants')
  }

  const profile = glslEs300Backend.capProfile
  const capabilities = Object.entries(profile).map(([id, support]) =>
    row(id, support?.directive ? `#extension ${support.directive} : require` : null, id, support?.hostFeature ?? ''),
  )
  ...
  const tables: Record<GlslTableId, readonly MappingRowData[]> = {
    ...
    samplers: [
      ...
      row(
        'sampler2DShadow',
        pick(shadow, 'sampler2DShadow shadowMap', 'shadow snippet').replace(/^uniform\s+(\S+)\s.*$/, '$1'),
        'texture_depth_2d',
      ),
      ...g(GLSL_FUSED),
    ],
    ...
    uniforms: [
      row('block', pickBlock(ubo, 'layout(std140) uniform Block {', '} u;', 'uniform snippet'), 'declare const u: uniform<Block>'),
      row('loose', null, 'declare const scale: uniform<f32>', looseMessage),
      row('storage', pick(storage, 'uniform sampler2D src;', 'storage snippet'), 'declare const src: storage<array<f32>>'),
      row('fetch', pick(storage, 'float _sfetch(', 'storage snippet'), 'src[i32(uv.x)]'),
      row('define', pick(override, '#define tint', 'override snippet'), 'const tint: override<f32> = 0.85'),
      row('precision', pick(ubo, 'precision highp float;', 'uniform snippet'), null),
    ],
    varyings: [
      row('out', pick(varyings.glslVertex, 'out vec2 uv;', 'varying snippet'), '@location(0) uv: vec2'),
      row('in', pick(varyings.glslFragment, 'in vec2 uv;', 'varying snippet'), '@location(0) uv: vec2'),
    ...
    variables: [
      ...
      row('glFragCoord', nameIn(fragCoord, 'gl_FragCoord', 'fragment snippet'), '@builtin("position")'),
      row('glVertexID', nameIn(vertexId, 'uint(gl_VertexID)', 'vertex snippet'), '@builtin("vertex_index")'),
      row('glInstanceID', nameIn(vertexId, 'uint(gl_InstanceID)', 'vertex snippet'), '@builtin("instance_index")'),
      row('glFrontFacing', nameIn(frontFacing, 'gl_FrontFacing', 'front facing snippet'), '@builtin("front_facing")'),
      row('glFragDepth', nameIn(frontFacing, 'gl_FragDepth', 'front facing snippet'), '@builtin("frag_depth")'),
    ],
    absentVariables: [
    ...
    operators: [
      row('mod', pick(operators, 'float floorMod', 'operator snippet'), 'const floorMod = mod(uv.x, 0.5)'),
      row('remainder', pick(operators, 'float truncMod', 'operator snippet'), 'const truncMod = uv.x % 0.5'),
      row('lessThan', pick(operators, 'bvec2 below', 'operator snippet'), 'const below: vec2b = uv < vec2(0.5, 0.25)'),
      row('equal', pick(operators, 'bvec2 same', 'operator snippet'), 'const same: vec2b = uv === vec2(0.75, 0.125)'),
      row('select', pick(operators, 'float picked', 'operator snippet'), 'const picked = select(uv.x, uv.y, below.x)'),
      row('vectorSelect', pick(operators, 'vec2 blended', 'operator snippet'), 'select(vec2(0., 0.), vec2(1., 1.), below)'),
    ],
  }
