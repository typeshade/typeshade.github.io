...
const numerals = (s: string): string =>
  (strip(s).match(/(?<![A-Za-z\d.`])\d+(?:\.\d+)*(?![A-Za-z\d])/g) ?? []).sort().join(' ');
const linkTargets = (s: string): string =>
  [...strip(s).matchAll(/\]\(([^)]+)\)/g)]
    .map((m) => m[1])
    .sort()
    .join(' ');
const headings = (s: string): number => (strip(s).match(/^#{2,6}\s/gm) ?? []).length;
const tableRows = (s: string): string[][] =>
  strip(s)
    .split('\n')
...
const KO_TELLS: Array<{ name: string; re: RegExp; max: number }> = [
  ...
  {
    name: 'bare 문 as a predicate (write 문(statement) at first use, then if 문 / raw 문 / 문장)',
    re: /(?:^|[^가-힣A-Za-z`])문(?:입니다|이며|이고|이라|이었)/g,
    max: 0,
  },
...
for (const locale of locales) {
  ...
  for (const t of guideTranslations(locale).values()) {
    ...
    for (const [k, n] of es)
      if ((ks.get(k) ?? 0) < n)
        push(
          `code span \`${k}\` appears ${n} time(s) in English, ${ks.get(k) ?? 0} in the translation`,
        );
    if (numerals(en) !== numerals(ko))
      push(`numerals differ: English [${numerals(en)}], translation [${numerals(ko)}]`);
    if (linkTargets(en) !== linkTargets(ko))
      push(`link targets differ: English [${linkTargets(en)}], translation [${linkTargets(ko)}]`);
    if (headings(en) !== headings(ko))
      push(`headings: ${headings(en)} in English, ${headings(ko)} in the translation`);
    ...
    if (badClose.length || badOpen.length)
      push(
        `bold markers CommonMark cannot parse: ${[...badClose, ...badOpen].slice(0, 3).join(' | ')}`,
      );
      ...
      er.forEach((row, i) => {
        row.forEach((cell, j) => {
          if (CODE_ONLY_CELL.test(cell) && kr[i][j] !== cell) {
            push(
              `table cell ${cell} (row ${i + 1}) must stay as it is; the translation has ${kr[i][j] ?? '(nothing)'}`,
            );
          }
        });
      });
    ...
    if (locale === 'ko')
      for (const w of FORBIDDEN)
        if (prose.includes(w))
          push(
            `"${w}" replaces a word the glossary keeps in English (GLOSSARY.md, 영어로 두는 낱말)`,
          );
    const latin = prose
      .split(/\n\s*\n/)
      .filter(
