...
oracle · every emitted variant compiled by Tint and by WebGL2 in CI · byte-stable emit goldens ·
emulated fp64 with unchanged syntax · `reflect()` recovering std140 / std430 layouts · 0 runtime
dependencies · 36 examples · 13 fp64 examples · 135 test files · in production inside the X-GIS
globe engine as `@xgis/shader-dsl` · MIT · `0.1.0` unreleased, npm `typeshade` reserved at `0.0.0`.
...
| --- | --- | --- |
| Target | TypeScript devs building WebGL2/WebGPU visuals; engine and library maintainers | Brief, *Audience* 1 and 2 |
| Need | The same shader on both APIs | Brief, "hand-written twin shaders are the tax" |
| Category | Engine-agnostic shader compiler for TypeScript | Brief, *Positioning sentence* |
| Key benefit | One typed source → WGSL + GLSL ES 3.00 | Brief, *Tagline of record* |
| Alternative | three.js TSL (tied to three); TypeGPU (WebGL2 backend experimental) | Brief, *Against incumbents* |
| Differentiation | CPU f64 oracle; Tint + WebGL2 compile gates in CI | Brief, *Proofs* |
| **Not claimed** | A renderer, a runtime, adapters, a playground, any customer beyond X-GIS | Not in the brief → forbidden |
...
rule stated as a method — the rejected v1 published the *feature* column.
...
| **Outcome** | Adding WebGPU stops being a second shader codebase to staff and maintain | You can ship a backend you cannot test on every machine you support | It drops into the renderer you already have — and already does, in the X-GIS globe engine |
...
| 9 | "0.1.0 is unreleased — is anyone actually using it?" | It runs in production inside the X-GIS globe engine as `@xgis/shader-dsl`, today | Hero status line, restated in section 6 |
...
| 8 | Two backends, one source of truth. (6) | Typed TypeScript becomes WGSL and GLSL ES 3.00, checked against a CPU oracle. (13) | **FAB** / 4 U's *Useful* | 4 | 4 | 3 | 5 | 16 |
...
>
> *under the CTAs:* 0.1.0 is not published yet. Running in production today inside the X-GIS globe engine.
> *nav chip (**P9**):* `pre-release`
> *caption under the live canvas:* the shader above, running on **WebGPU** *(substituted at runtime: WebGPU / WebGL2 / still frame)*

**4 U's check** — *Useful*: yes, it names the job. *Ultra-specific*: carried by the subline's
`WGSL` / `GLSL ES 3.00`, not the headline. *Unique*: moderate; the subline's "proven to agree"
supplies it. *Urgent*: absent, deliberately — urgency on a pre-release compiler would be dishonest.
...
| 3° | `Read the CI gates` | Docs / test corpus | Consideration | Section 3 | Lets a sceptic verify the differentiator instead of believing it |
...
| — | *(none)* | — | **Revenue, Retention, Referral** | — | Nothing is for sale, `0.1.0` is unpublished, and there is no account. A `Pricing` or `Sign up` CTA would be dishonest, so those funnel stages are deliberately unserved |
...
- **Body** — That is the whole source for the shader running above. Both targets come out of it, byte for byte the same every build. *(23 words)*
...
- **Micro-label** — `CPU f64 oracle · Tint · WebGL2` · **CTA** `Read the CI gates` · **Pillar P-2** · **Answers objection 4**

### 4 · The editor catches it — *capability*
...
- **Body** — The GPU has no f64. The emulation does, and the source reads exactly like the f32 version. *(17 words)*
- **Micro-label** — `emulated fp64` · **CTA** `Open the deep-zoom example` · **Answers objection 7**

### 6 · Where it runs — *close*
- **Purpose** — answer "is it welded to an engine, and is anyone using it" honestly, then close on the real CTA.
- **Claim** — No engine. No dependencies.
- **Proof shown** — `0 runtime dependencies` · `@xgis/shader-dsl` · `MIT` · the pinned commit; the CTA pair repeated.
...
- **Columns** — **Start**: `Authoring guide`, `Docs` · **Source**: `GitHub mirror`, `X-GIS source` · **Package**: `npm typeshade — reserved for 0.1.0` · **Agents**: `llms.txt`
- **Body** — TypeShade · MIT · compiled from the mirror pinned at *(commit)*.
...
*From the brief* — we · our · simply · easily · just · powerful.

*Added* — seamless(ly) · blazing / blazingly · lightning-fast · fast (without a number) · robust ·
...
| 4 | "The compiler is in production inside the X-GIS globe engine as @xgis/shader-dsl. The typeshade npm package is reserved until the 0.1.0 release. Until then, add the read-only mirror as a submodule." | **Running in a globe engine today.** 0.1.0 is not published yet — start from the mirror. *(`@xgis/shader-dsl` as the chip beside it)* | Apology → proof; 33 words → 15 |
...
- Link text names its destination as a noun: `Authoring guide`, `GitHub mirror`, `X-GIS source`. Never `here`, `read more`, `click here`, `learn more`, or a bare URL in body copy.
