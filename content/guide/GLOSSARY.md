...
| invocation | 인보케이션 | GPU가 진입점을 한 번 실행하는 단위 하나. "호출"은 함수 호출(call)에 이미 쓰고 있고 "실행 단위"는 워크그룹·디스패치와 섞이므로, WGSL 명세의 낱말을 그대로 음차합니다. 처음 등장할 때 "한 번 실행되는 단위를 인보케이션이라고 부릅니다"처럼 뜻을 먼저 풀고 이후에는 이름으로 씁니다. 사이트의 `/guide/concepts/cpu-and-gpu/` 페이지가 쓰는 이름입니다. |
| work item | 작업 항목 | 컴퓨트 디스패치에서 인보케이션 하나가 맡는 항목. "워크 아이템"이라는 음차보다 뜻이 바로 읽히고, 이미 고정한 "디스패치"와 함께 써도 헷갈리지 않습니다. |
| dispatch grid | 디스패치 격자 | 컴퓨트 디스패치가 만드는 작업 항목의 삼차원 배열. "그리드"보다 "격자"가 한국어 수치 문서에서 더 널리 쓰입니다. "디스패치"는 이미 고정한 용어입니다. |
| rasterizer | 래스터라이저 | 프리미티브가 덮는 프래그먼트를 가려내고 버텍스가 만든 값을 프래그먼트마다 가중하는 고정 단계. 한국어 그래픽스 문서에서 이미 음차로 굳어졌습니다. |
| primitive (graphics) | 프리미티브 | 삼각형·선·점처럼 래스터라이저가 다루는 도형 하나. 한국어 그래픽스 문서의 음차를 따릅니다. |
| interpolation (between stages) | 보간 | 버텍스가 만든 값을 프래그먼트 위치에 맞게 가중해 넘기는 것. 수학과 그래픽스 문서에서 이미 쓰는 번역어이며, 위 "varying" 행과 짝을 이룹니다. |
| clip space | 클립 공간 | 버텍스 스테이지가 돌려주는 위치가 놓이는 좌표계. 한국어 그래픽스 문서의 표기를 따릅니다. |
| heap | 힙 | 실행 중에 메모리를 할당받는 영역. 한국어 개발 문서에서 이미 음차로 굳어졌습니다. |
| call stack | 호출 스택 | 함수가 돌아갈 자리를 쌓아 두는 스택. 이미 있는 "호출 그래프"와 짝을 맞췄습니다. |
| flatten (call graph) | 평탄화 | 호출 그래프의 호출을 모두 펼쳐 없애는 것. `inline`이 하는 일이며, 한국어 컴파일러 문서에서 쓰는 말입니다. |
| unroll (loop) | 펼치다 | 루프 본문을 반복 횟수만큼 늘어놓는 것. 명사형이 필요하면 "루프 펼치기"를 씁니다. |
