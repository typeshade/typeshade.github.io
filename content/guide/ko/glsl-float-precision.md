...
기본값은 스테이지 전체에 적용되므로 위치, 월드 단위 값, varying은 물론 이 옵션을 고를
때는 미처 생각하지 못했을 스테이지 안의 다른 중간값까지 모두 영향을 받습니다. 유효
숫자가 세 자리를 넘게 필요한 값은 버티지 못하는데, 월드 단위로 나타낸 위치가 그런 값의
한 예입니다. f32 자체도 값이 일곱 자리를 넘어서면 이미 무너지며,
...
정밀도가 아예 없으므로 이 줄은 반드시 있어야 하며, 데이터 텍스처를 통해 스토리지 버퍼를
읽는 인덱스 연산과 비트캐스트의 정수 쪽 절반 모두 int의 전체 범위를 필요로 합니다. 이
정밀도를 낮추면
