...
크기가 같은 **벡터 하나**를 통째로 받으면 생성자는 조합하지 않고 변환합니다. `vec3<u32>` 값에 대한
`vec3(v)`는 WGSL의 `vec3<f32>(v)`이며, 구성 요소마다 스칼라 변환을 거칩니다. 내보내는 코드가
그렇고, 이제는 CPU에서 계산하는 값도 같습니다. 예전 오라클은 원본 구성 요소를 그대로 넘겼기
때문에 `vec3u(vec3(1.7, 2.9, -3.2))`가 `[1.7, 2.9, -3.2]`로 읽혔지만, WGSL은 `[1, 2, 0]`을 냅니다.
부동소수점 원본은 정수 타깃에서 포화되고, `i32`와 `u32`는 이의 보수로 재해석됩니다. GLSL ES 3.00에서는
범위 안의 원본만 정의되어 있습니다.
