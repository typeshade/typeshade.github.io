...
**같은 크기의 벡터 하나**를 통째로 넘기면 생성자는 구성 요소를 모으지 않고 변환합니다.
`vec3<u32>` 값에 쓴 `vec3(v)`는 WGSL의 `vec3<f32>(v)`이고, 성분마다 스칼라 변환을 거칩니다.
생성되는 코드가 그렇고, 이제는 CPU에서 계산하는 값도 같습니다. 예전 오라클은 원본 성분을
그대로 통과시켜서 `vec3u(vec3(1.7, 2.9, -3.2))`가 `[1.7, 2.9, -3.2]`로 읽혔지만, WGSL이
주는 값은 `[1, 2, 0]`입니다. 부동소수점 원본은 정수 타입으로 갈 때 포화되고, `i32`와 `u32`는
같은 비트열을 부호 있는 값과 부호 없는 값으로 서로 바꿔 해석합니다. GLSL ES 3.00에서는
범위 안에 있는 원본만 정의되어 있습니다.
